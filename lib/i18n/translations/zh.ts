/**
 * Chinese Simplified (简体中文) translations
 */

export default {
    common: {
        loading: '加载中...',
        error: '错误',
        success: '成功',
        cancel: '取消',
        save: '保存',
        delete: '删除',
        edit: '编辑',
        done: '完成',
        next: '下一步',
        back: '返回',
        skip: '跳过',
        search: '搜索',
        retry: '重试',
        close: '关闭',
        confirm: '确认',
        yes: '是',
        no: '否',
        and: '和',
        change: '修改',
        untitled: '无标题',
        yesterday: '昨天',
        info: '提示',
        completed: '已完成',
        progress: '进度',
        unknownLocation: '未知位置',
        permissionRequired: '需要权限',
        offlineMode: '离线模式',
        pullToRefresh: '下拉刷新',
        releaseToRefresh: '松开刷新',
        percentValue: '{{value}}%',
        ok: '好',
    },
    auth: {
        login: '登录',
        signup: '注册',
        logout: '退出',
        email: '邮箱',
        password: '密码',
        confirmPassword: '确认密码',
        forgotPassword: '忘记密码？',
        resetPassword: '重置密码',
        welcomeBack: '欢迎回来！',
        createAccount: '创建账户',
        alreadyHaveAccount: '已有账户？',
        dontHaveAccount: '没有账户？',
        orContinueWith: '或使用以下方式继续',
        orSignupWith: '或使用以下方式注册',
        loginSuccess: '登录成功',
        signupSuccess: '账户创建成功',
        logoutSuccess: '已退出',
        invalidCredentials: '邮箱或密码无效',
        emailRequired: '请输入邮箱',
        passwordRequired: '请输入密码',
        passwordTooShort: '密码至少需要6个字符',
        tagline: '探索世界，分享你的故事。',
        startJourney: '今天开始你的旅程。',
        emailOrUsername: '邮箱或用户名',
        emailAddress: '邮箱地址',
        fullName: '姓名',
        passwordMinChars: '密码（6个字符以上）',
        enterEmailPassword: '请输入邮箱和密码。',
        fillAllFields: '请填写所有字段。',
        passwordsDontMatch: '密码不匹配。',
        passwordMinLength: '密码至少需要6个字符。',
        acceptTermsRequired: '请同意服务条款。',
        loginError: '登录错误',
        signupError: '注册错误',
        accountCreated: '账户已创建！',
        checkEmailVerify: '请检查邮箱进行验证。',
        comingSoon: '即将推出',
        googleLoginSoon: 'Google登录即将推出！',
        appleLoginSoon: 'Apple登录即将推出！',
        googleSignupSoon: 'Google注册即将推出！',
        appleSignupSoon: 'Apple注册即将推出！',
        termsOfService: '服务条款',
        privacyPolicy: '隐私政策',
        acceptSuffix: '。',
        copyright: '© 2026 Odyssey Journal. 版权所有。',
        forgotPasswordTitle: '忘记密码',
        forgotPasswordSubtitle: '别担心。输入邮箱重置密码。',
        enterEmail: '请输入您的邮箱地址。',
        invalidEmail: '请输入有效的邮箱。',
        emailSent: '邮件已发送！',
        resetLinkSentTo: '重置链接已发送至：',
        checkInboxSpam: '请检查收件箱。也检查垃圾邮件。',
        backToLogin: '返回登录',
        resendEmail: '没收到？重新发送',
        sendResetLink: '发送链接',
        rememberPassword: '想起来了？',
        resetLinkValidity: '为了安全，链接1小时内有效。',
        newPassword: '新密码',
        enterNewPassword: '输入您的新密码',
        confirmNewPassword: '确认新密码',
        confirmNewPasswordPlaceholder: '再次输入新密码',
        passwordLengthError: '密码至少需要6个字符。',
        passwordMatchError: '密码不匹配。',
    },
    onboarding: {
        step1Title: '开始探索',
        step1Desc: '发现世界上最迷人的路线，获取旅行贴士，分享你难忘的回忆。',
        step2Title: '分享你的\n回忆',
        step2Desc: '分享旅行中的照片和故事。与其他旅行者连接。',
        step3Title: '获取灵感\n并激励他人',
        step3Desc: '从我们的社区获取灵感，并用你的冒险激励他人。',
        skip: '跳过',
        start: '开始',
        next: '下一步',
    },
    home: {
        title: 'Odyssey Journal',
        noAdventures: '暂无冒险',
        startSharing: '开始分享你的旅行回忆吧！',
        createFirstPost: '创建第一篇帖子',
        refreshing: '刷新中...',
        publicFeed: '公开动态',
        followingFeed: '关注动态',
        noFollowingPosts: '暂无动态',
        exploreTravelers: '在社区中探索其他旅行者并关注他们，以便在这里查看他们的动态。',
        exploreButton: '发现旅行者',
        sortLatest: '最新',
        sortPopular: '热门',
        sortLocation: '有位置',
        sortLatestAccessibility: '按最新帖子排序',
        sortPopularAccessibility: '按热门帖子排序',
        sortLocationAccessibility: '仅显示有位置的帖子',
    },
    create: {
        title: '新帖子',
        newEntry: '新日记',
        whatsOnMind: '你在想什么？',
        addPhoto: '添加照片',
        addLocation: '添加位置',
        post: '发布',
        posting: '发布中...',
        postSuccess: '帖子已分享',
        postError: '分享失败',
        titleRequired: '请输入标题',
        contentRequired: '请写下你的故事',
        chooseOption: '选择一个选项',
        takePhoto: '拍照',
        pickFromGallery: '从相册选择',
        discardChanges: '放弃更改？',
        addNote: '添加备注...',
        confirmLocation: '确认位置',
        changeLocation: '更改',
        sealingMemories: '正在封存回忆...',
        unknownLocation: '未知位置',
        discardMessage: '所有更改将丢失。',
        keepEditing: '继续编辑',
        discard: '放弃',
        selectImages: '选择照片',
        maxImages: '最多可选择{{count}}张照片',
        writeCaption: '写个说明...',
        categories: '分类',
        images: '图片',
        location: '位置',
        locationSelected: '已选择位置',
        tapToAddLocation: '点击添加位置',
        titlePlaceholder: '标题',
        storyPlaceholder: '开始讲述你的故事...',
        emailNotVerifiedTitle: '需要进行电子邮件验证',
        emailNotVerifiedDesc: '请验证您的电子邮件地址，以便与社区分享新的日记条目。',
        searchPlaceholder: '搜索城市或地点...',
        useCurrentLocation: '使用当前位置 (GPS)',
        noResults: '未找到结果。请再试一次。',
        locationNotFound: '未找到位置。请尝试其他搜索。',
        locationSearchError: '搜索位置时发生错误。',
    },
    explore: {
        title: '探索',
        searchPlaceholder: '搜索地点、用户或标签...',
        trending: '热门',
        categories: '分类',
        users: '用户',
        places: '地点',
        noResults: '无结果',
        tryDifferent: '试试其他搜索词',
        popularPosts: '热门帖子',
        viewAll: '查看全部',
        popularDestinations: '热门目的地',
        routes: '路线',
        weeklyDiscovery: '每周发现',
        editorsChoice: '编辑精选',
        suggestedTravelers: '推荐旅行者',
        follow: '关注',
        loading: '加载中...',
        loadFailed: '加载失败',
        noSuggestionsYet: '暂无推荐',
        recentSearches: '最近搜索',
        locations: '地点',
        posts: '帖子',
        discoverRoutes: '发现路线...',
        searchCities: '搜索城市...',
        findTravelers: '寻找旅行者...',
        discoverPlaces: '发现新地点...',
        unknown: '未知',
        untitledPost: '无标题帖子',
        user: '用户',
        category: '分类',
        noCategoryPosts: '该分类下暂无帖子',
        readPost: '阅读帖子',
        newRoutesToExplore: '探索新路线',
        featuredContentDefault: '发现世界上最美丽的风景，开启全新冒险。',
        viewCategoryPosts: '查看{{category}}分类中的帖子',
        featuredDefaultTitle: '探索新路线',
        featuredDefaultDescription: '探索最美丽的路线，踏上奇妙冒险。',
        postCount: '{{count}} 篇帖子',
    },
    categories: {
        nature: '自然',
        city: '城市',
        food: '美食',
        history: '历史',
        art: '艺术',
        adventure: '冒险',
        beach: '海滩',
        culture: '文化',
        mountain: '山脉',
        wildlife: '野生动物',
    },
    collection: {
        saveToCollection: '保存到收藏夹',
        myCollections: '我的收藏夹',
        nameInputPlaceholder: '收藏夹名称...',

        createTitle: '创建合集',
        editTitle: '编辑合集',
        save: '保存',
        coverPhoto: '封面照片',
        addPhoto: '添加照片',
        collectionName: '合集名称',
        namePlaceholder: '例如：2024年夏天',
        themeColor: '主题颜色',
        preview: '预览',
        errorTitle: '错误',
        nameRequired: '请输入合集名称',
        updateError: '无法更新合集',
        createError: '无法创建合集',
        permissionRequired: '需要权限',
        galleryPermission: '需要访问相册',
        zeroPosts: '0 帖子',
        deleteTitle: '删除合集',
        deleteConfirm: '确定要删除"{{name}}"吗？已保存的帖子不会被删除。',
        deleteError: '无法删除合集',
        loadError: '无法加载合集',
        empty: '合集为空',
        emptyDesc: '此合集还没有添加帖子',
        notFound: '未找到合集',
        posts: '帖子',
        details: '详情',
        noNote: '此帖子没有添加备注。',
        unknownLocation: '未知位置',
        untitled: '无标题',
        noNotes: '此动态暂无备忘笔记。',
        saveWithoutCollection: '不加入合集直接保存',
        saveError: '保存时发生错误',
        create: '创建',
    },

    // Chat & Messaging
    chat: {
        title: '信件',
        noConversations: '您的收件箱是空的',
        startConversation: '在任何旅行者的个人资料上点击“发送消息”来写您的第一封信。',
        loading: '正在加载消息...',
        typeMessage: '写信...',
        emptyChat: '发送您的第一条消息',
        error: '错误',
        requests: '请求',
        primaryInbox: '信件',
        accept: '接受',
        decline: '删除',
        requestBanner: '您想与此用户聊天吗？在您接受之前，您将无法回复。',
        declineConfirm: '您确定要拒绝此请求吗？聊天记录将被删除。',
        noRequests: '没有找到消息请求。',
        openLettersAndChats: '打开信件和聊天',
        typing: '正在输入...',
        yesterday: '昨天',
    },

    saved: {
        title: '已保存',
        noSaved: '暂无保存的帖子',
        saveDescription: '保存喜欢的帖子以便稍后查看',
        browseExplore: '浏览探索',
        loadError: '无法加载保存的帖子',
        posts: '帖子',
        unknownLocation: '未知位置',
        untitled: '无标题',
        details: '详情',
        noNote: '此帖子无备注。',
        collections: '合集',
        new: '新建',
        noCollectionsYet: '暂无合集',
        createCollectionHint: '创建合集来整理',
        noSavedPosts: '暂无保存的帖子',
        savedPostsWillAppear: '保存的帖子将显示在这里',
    },
    profile: {
        title: '个人资料',
        editProfile: '编辑资料',
        followers: '粉丝',
        following: '关注',
        posts: '帖子',
        settings: '设置',
        myJourney: '我的旅程',
        countries: '国家',
        kilometers: '公里',
        days: '天',
        badges: '徽章',
        journalNotes: '日记笔记',
        noPostsYet: '暂无帖子',
        startJourney: '开始你的旅程',
        logoutTitle: '退出登录',
        logoutMessage: '确定要退出登录吗？',
        cancel: '取消',
        logout: '退出',
        journalNote: '日记笔记',
        republicOf: '共和国',
        noJournalNotes: '暂无日记笔记',
        shareMemories: '用照片分享您的旅行回忆',
        addFirstNote: '添加第一条笔记',
        travelMap: '旅行地图',
        noTravelLocation: '暂无旅行地点',
        shareLocationPosts: '分享带位置的帖子',
        passport: '护照',
        approvedTraveler: '认证旅行者',
        message: '发消息',
        commonDestinations: '共同目的地',
        commonPlaces: '共同地点',
        noCommonDestinations: '暂无共同目的地',
        noCommonDestinationsSubtext: '继续探索世界吧！',
        openMap: '打开地图',
        travelJournal: '旅行日记',
        defaultUser: '旅行者',
        surnameTitle: '姓',
        nameTitle: '名',
        nationalityTitle: '国籍',
        blockUserTitle: '屏蔽用户',
        blockUserDesc: '您确定要屏蔽此用户吗？他们将无法与您互动，并且他们的帖子将从您的动态中隐藏。',
        block: '屏蔽',
        blockSuccess: '成功屏蔽用户。',
        blockError: '屏蔽用户失败。',
        notFoundTitle: '未找到旅行者',
        notFoundDesc: '此个人资料不可用，或者您没有权限查看。',
        natureLover: '大自然爱好者',
        cityExplorer: '城市探寻者',
        historian: '历史探索者',
        photographer: '摄影爱好者',
        traveler: '旅行者',
    },
    editProfile: {
        title: '编辑资料',
        cancel: '取消',
        save: '保存',
        fullName: '姓名',
        fullNamePlaceholder: '请输入您的姓名',
        username: '用户名',
        usernamePlaceholder: '请输入用户名',
        bio: '简介',
        bioPlaceholder: '介绍一下自己',
        website: '网站',
        websitePlaceholder: 'https://yourwebsite.com',
        changePhoto: '更换头像',
        saveChanges: '保存更改',
        saving: '保存中...',
        saved: '已保存',
        updateSuccess: '资料更新成功',
        updateError: '资料更新失败',
        permissionNeeded: '需要权限',
        permissionMessage: '请授权访问您的照片',
    },
    notifications: {
        title: '通知',
        noNotifications: '暂无通知',
        checkBack: '稍后再来查看',
        today: '今天',
        thisWeek: '本周',
        older: '更早',
        markAllRead: '全部标为已读',
        likedPost: "喜欢了 '{{postTitle}}'",
        commented: '评论了 {{postTitle}}',
        followed: '关注了你',
        mentioned: '提到了你',
        inboxUpToDate: '收件箱已是最新',
        loadError: '无法加载通知',
        markAllError: '无法全部标为已读',
        newNotification: '新通知',
        nearbyRoute: '附近发现新路线: {{locationName}}',
        someone: '有人',
    },
    comments: {
        title: '评论',
        noComments: '暂无评论',
        beFirst: '成为第一个评论的人！',
        addComment: '添加评论...',
        posting: '发布中...',
        deleteComment: '删除评论',
        deleteConfirm: '确定要删除这条评论吗？',
        loadError: '无法加载评论',
        addError: '无法添加评论',
        deleteError: '无法删除评论',
    },
    post: {
        like: '喜欢',
        unlike: '取消喜欢',
        comment: '评论',
        share: '分享',
        save: '保存',
        unsave: '取消保存',
        delete: '删除',
        report: '举报',
        deleteConfirm: '确定要删除这篇帖子吗？',
        deleteSuccess: '帖子已删除',
        reportSuccess: '举报已提交',
        editTitle: '编辑帖子',
        deleteTitle: '删除帖子',
        address: '地址',
        memoriesTitle: '回忆与笔记',
        memory: '回忆',
        notesCount: '{{count}} 条笔记',
        reportPost: '举报帖子',
        viewNotes: '查看 {{count}} 条笔记',
        writeNote: '为日记写一条笔记...',
        noNotes: '暂无笔记',
        beFirst: '成为第一个添加回忆的人！',
        deleteError: '删除帖子失败',
        location: '位置',
        boardingPass: '登机牌',
    },
    report: {
        title: '举报帖子',
        whyReporting: '为什么要举报这篇帖子？',
        additionalDetails: '补充详情（可选）',
        detailsPlaceholder: '提供更多信息...',
        submit: '提交举报',
        success: '举报已收到',
        reasons: {
            spam: { label: '垃圾信息', desc: '重复或无关内容' },
            harassment: { label: '骚扰或欺凌', desc: '针对或攻击他人' },
            hate_speech: { label: '仇恨言论', desc: '歧视性或冒犯性内容' },
            violence: { label: '暴力或威胁', desc: '宣扬或威胁暴力' },
            nudity: { label: '裸露或色情内容', desc: '不当色情内容' },
            false_information: { label: '虚假信息', desc: '误导或虚假内容' },
            other: { label: '其他', desc: '其他原因' },
        },
        alreadyReportedTitle: '已举报',
        alreadyReportedDesc: '您已举报过这篇帖子，我们会尽快审核。',
        submittedTitle: '举报已提交',
        submittedDesc: '感谢您的举报，我们会审核并采取相应措施。',
        submitFailed: '举报提交失败，请重试。',
    },
    follow: {
        follow: '关注',
        following: '已关注',
        unfollow: '取消关注',
        followers: '粉丝',
        followingList: '关注',
        noFollowers: '暂无粉丝',
        noFollowing: '暂未关注任何人',
    },
    settings: {
        title: '设置',
        account: '账户',
        notifications: '通知',
        privacy: '隐私',
        appearance: '外观',
        language: '语言',
        languageOptions: '语言选项',
        darkMode: '深色模式',
        about: '关于',
        help: '帮助',
        terms: '服务条款',
        privacyPolicy: '隐私政策',
        version: '版本',
        logout: '退出',
        deleteAccount: '删除账户',
        deleteAccountConfirm: '确定要删除账户吗？此操作无法撤销。\n\n如需在删除前获取数据副本，请发送邮件至 privacy@odysseyjournal.app，我们会为您导出。',
        deleteAccountWarning: '您的所有帖子、评论和数据将被永久删除。',
        blockedUsers: '已屏蔽的用户',
        blockedUsersDesc: '管理和解除屏蔽您已屏蔽的用户',
        noBlockedUsers: '没有已屏蔽的用户。',
        unblockConfirmTitle: '解除屏蔽',
        unblockConfirmDesc: '您确定要解除屏蔽此用户吗？',
        unblockSuccess: '用户已解除屏蔽。',
        deleteAccountButton: '删除我的账户',
        accountDeleted: '您的账户已被删除。',
        logoutConfirm: '确定要退出吗？',
        passportHolder: '护照持有人',
        theme: '主题',
        lightDark: '浅色 / 深色',
        exitLabel: '退出',
        exportDataTitle: '申请我的数据',
        exportDataDesc: '如需获取您的个人数据副本，请发送邮件至 privacy@odysseyjournal.app。我们会导出您的个人资料、帖子、评论、收藏和关注关系并发送给您。',
        exportDataSubdesc: '发送邮件申请个人数据副本',
        sendEmail: '发送邮件',
        legal: '法律与社区',
        communityGuidelines: '社区准则',
        communityGuidelinesDesc: '维护社区安全的准则',
        adminTitle: '管理员',
        moderationPanel: '审核面板',
        moderationPanelDesc: '审查举报，管理用户',
        likeNotifications: '赞',
        likeNotificationsDesc: '当有人喜欢您的动态时接收通知',
        commentNotifications: '评论',
        commentNotificationsDesc: '当有人评论您的动态时接收通知',
        followNotifications: '关注者',
        followNotificationsDesc: '当有人开始关注您时接收通知',
        changePassword: '修改密码',
        changePasswordDesc: '更新您的账户密码',
        preferencesUpdateError: '更新偏好设置时发生错误。',
        notificationPreferences: '通知偏好设置',
        passwordChangeSuccess: '您的密码已成功更新。',
        unblockError: '解除屏蔽用户失败。',
        build: '构建',
        restartTitle: '需要重新启动',
        restartMessage: '请关闭并重新打开 Odyssey Journal，以完成界面方向的切换。',
    },
    errors: {
        generic: '出错了',
        network: '连接错误',
        tryAgain: '请重试',
        sessionExpired: '会话已过期',
        notFound: '未找到',
        unauthorized: '未授权',
        serverError: '服务器错误',
        boundaryTitle: '我们的指南针失效了！',
        boundarySubtitle: '发生了意外错误。我们正在努力恢复正常。',
        boundaryErrorDetails: '错误详情',
        boundaryRetry: '重启应用',
        boundaryShowDetails: '显示错误详情',
        boundaryHideDetails: '隐藏错误详情',
        noConnection: '无网络连接',
        permissionPhotos: '上传图片需要访问相册的权限。',
        permissionCamera: '拍照需要使用相机的权限。',
        permissionLocation: '为帖子添加位置需要定位权限。',
        imagePickFailed: '选择图片失败，请重试。',
        imagesPickFailed: '选择图片失败，请重试。',
        photoCaptureFailed: '拍照失败，请重试。',
        locationFailed: '无法获取当前位置，请重试。',
        signInTitle: '登录错误',
        signInProviderFailed: '使用 {{provider}} 登录时出现问题。',
        shareFailed: '分享帖子失败',
        emailNotConfirmed: '请先验证您的邮箱地址后再登录',
        emailExists: '该邮箱已注册过账号',
        weakPassword: '请设置更强的密码',
        samePassword: '新密码不能与当前密码相同',
        rateLimit: '尝试次数过多，请稍后再试',
        linkExpired: '该链接已过期，请重新获取',
        userNotFound: '未找到使用该邮箱的账号',
        signupDisabled: '目前暂不开放新用户注册',
        usernameTaken: '该用户名已被使用',
    },
    time: {
        now: '刚刚',
        minuteAgo: '{{count}}分钟前',
        minutesAgo: '{{count}}分钟前',
        hourAgo: '{{count}}小时前',
        hoursAgo: '{{count}}小时前',
        dayAgo: '{{count}}天前',
        daysAgo: '{{count}}天前',
        weekAgo: '{{count}}周前',
        weeksAgo: '{{count}}周前',
        monthAgo: '{{count}}个月前',
        monthsAgo: '{{count}}个月前',
        yearAgo: '{{count}}年前',
        yearsAgo: '{{count}}年前',
        yesterday: '昨天',
        lastWeek: '上周',
    },
    empty: {
        noData: '暂无数据',
        noPosts: '暂无帖子',
        noUsers: '暂无用户',
        noPlaces: '暂无地点',
    },
    map: {
        title: '地图',
        loading: '地图加载中...',
        notSupportedWeb: '地图在网页版不可用',
        notSupportedExpo: '地图在Expo Go中不可用',
        useMobileApp: '请使用移动应用查看地图。',
        useDevBuild: '查看地图需要开发版本。',
        fallbackList: '您可以在下方查看位置列表。',
        stats: '{{locationCount}} 地点 • {{postCount}} 帖子',
        unknownLocation: '未知位置',
        postCount: '{{count}} 帖子',
        noPosts: '暂无带位置的帖子',
        addLocationTip: '给你的帖子添加位置',
        openInMap: '在地图中打开',
        travelPoint: '旅行地点',
        journeyRoute: '旅程路线 ({{count}}个地点)',
        noJourneyYet: '尚无旅程',
        startExploring: '开始探索世界！',
        notSupported: '不支持地图',
        locationVisited: '已访问地点',
        locationsVisited: '已访问地点',
        appleMaps: 'Apple 地图',
        openInMaps: '地图',
    },

    destinationPosts: {
        empty: '此位置暂无帖子',
        emptyDesc: '此目的地暂无分享内容',
        destination: '目的地',
        untitledPost: '无标题帖子',
        user: '用户',
        error: '发生错误',
    },
    followers: {
        title: '关注者',
        noFollowers: '暂无关注者',
        noFollowersDesc: '当有人关注此用户时，他们将显示在这里',
        loadError: '加载关注者失败',
        followError: '更新关注状态失败',
    },
    following: {
        title: '正在关注',
        notFollowing: '未关注任何人',
        notFollowingDesc: '当此用户关注他人时，他们将显示在这里',
        loadError: '加载关注列表失败',
        followError: '更新关注状态失败',
    },
    popularPosts: {
        title: '热门帖子',
        empty: '暂无热门帖子',
        emptyDesc: '暂未发现热门帖子',
        untitledPost: '无标题帖子',
        user: '用户',
        error: '发生错误',
    },
    createPost: {
        newEntry: '新日记条目',
        cancel: '取消',
        share: '分享',
        images: '图片',
        addPhoto: '添加照片',
        addNote: '添加备注...',
        confirmLocation: '确认位置',
        changeLocation: '更改',
        titlePlaceholder: '标题',
        storyPlaceholder: '开始讲述你的故事...',
        errorTitle: '错误',
        enterTitle: '请输入标题',
        enterStory: '请写下你的故事',
        success: '成功',
        entryCreated: '你的日记条目已创建！',
        ok: '确定',
        createError: '无法创建帖子，请重试。',
        addPhotoTitle: '添加照片',
        chooseOption: '选择一个选项',
        takePhoto: '拍照',
        pickFromGallery: '从相册选择',
        nature: '自然',
        city: '城市',
        food: '美食',
        history: '历史',
        art: '艺术',
        emailNotVerifiedTitle: '需要进行电子邮件验证',
        emailNotVerifiedDesc: '请验证您的电子邮件地址，以便与社区分享新的日记条目。',
    },

    weather: {
        clear: '晴朗',
        mainlyClear: '大部分晴朗',
        partlyCloudy: '局部多云',
        overcast: '阴天',
        foggy: '有雾',
        depositingRimeFog: '雾凇',
        lightDrizzle: '毛毛雨',
        moderateDrizzle: '中等毛毛雨',
        denseDrizzle: '大毛毛雨',
        freezingDrizzle: '冻毛毛雨',
        denseFreezingDrizzle: '大冻毛毛雨',
        slightRain: '小雨',
        moderateRain: '中雨',
        heavyRain: '大雨',
        freezingRain: '冻雨',
        heavyFreezingRain: '大冻雨',
        slightSnow: '小雪',
        moderateSnow: '中雪',
        heavySnow: '大雪',
        snowGrains: '雪粒',
        slightRainShowers: '小阵雨',
        moderateRainShowers: '中阵雨',
        violentRainShowers: '大阵雨',
        slightSnowShowers: '小阵雪',
        heavySnowShowers: '大阵雪',
        thunderstorm: '雷暴',
        thunderstormSlightHail: '轻微冰雹雷暴',
        thunderstormHeavyHail: '强冰雹雷暴',
        unknown: '未知',
    },
    guidelines: {
        intro: '欢迎来到 Odyssey Journal！我们希望保持这个空间安全、充满灵感，并专注于真实的旅行体验。请遵守以下准则。',
        respectTitle: '尊重社区',
        respectDesc: 'Odyssey Journal 是旅行者分享真实体验的地方。请尊重他人。严禁任何仇恨言论、骚扰、欺凌和歧视。',
        authenticTitle: '分享真实的旅行',
        authenticDesc: '专注于旅行、地点、文化和您的个人旅程。请勿发布垃圾信息、广告、政治宣传或无关内容。',
        safeTitle: '保持安全和适宜',
        safeDesc: '请勿发布裸体、性暗示内容或暴力血腥画面。确保您的照片和文字适合大众受众。',
        privacyTitle: '尊重隐私',
        privacyDesc: '未经他人明确同意，请勿分享他人的个人信息。注意您在照片中拍摄的人物和事物。',
        enforcementTitle: '违规处理',
        enforcementDesc: '我们会审查被举报的内容和帐户。违反这些准则可能会导致内容被删除、访问受限或帐户被永久删除，恕不另行通知。',
        thankYou: '感谢您帮助我们使 Odyssey Journal 成为旅行者的绝佳场所！'
    },
    // Content Moderation
    moderation: {
        flaggedNoReasons: '您的内容已被标记以待审查。请确保它符合我们的社区准则。',
        flaggedWithReasons: '您的内容因以下原因被标记: {{reasons}}。请确保您的帖子符合我们的社区准则。',
        categories: {
            harassment: '骚扰',
            'harassment/threatening': '威胁性骚扰',
            hate: '仇恨言论',
            'hate/threatening': '威胁性仇恨言论',
            'self-harm': '自残',
            'self-harm/intent': '自残意图',
            'self-harm/instructions': '自残指示',
            sexual: '色情内容',
            'sexual/minors': '未成年人色情内容',
            violence: '暴力',
            'violence/graphic': '血腥暴力画面'
        }
    },
    homeLocation: {
        title: '设置常居地',
        description: '设置您的常居地，以便我们准确计算您的总旅行里程和护照统计数据。',
        useCurrentLocation: '使用当前位置',
        locating: '正在获取位置...',
        later: '稍后',
        success: '常居地设置成功。',
        permissionDenied: '未授予位置权限。您可以稍后在个人资料中进行设置。',
        error: '获取位置失败。',
    },
    badgeList: {
        firstAdventure: {
            name: "初次冒险",
            description: "分享你的第一条动态"
        },
        worldTraveler: {
            name: "环球旅行家",
            description: "探访5个不同的国家"
        },
        photographer: {
            name: "摄影大师",
            description: "分享10条带有照片的动态"
        },
        gourmet: {
            name: "美食家",
            description: "在美食分类分享3条动态"
        },
        marathonTraveler: {
            name: "马拉松行者",
            description: "旅行里程达10,000公里"
        },
        explorer: {
            name: "探险家",
            description: "旅行达30天"
        }
    },
    // Date Picker
    datePicker: {
        title: '日期 / 旅行时间',
        tapToChange: '点击修改日期',
        change: '修改',
        today: '今天',
        yesterday: '昨天',
        tomorrow: '明天',
        threeDaysAgo: '3天前',
        calendar: '日历',
        selectDate: '选择旅行日期',
        selected: '已选:',
        apply: '应用日期',
        cancel: '取消',
        daysAgo: '{{count}}天前',
        daysLater: '{{count}}天后',
    },

    // Admin panel
    admin: {
        panelTitle: '管理面板',
        accessDenied: '访问被拒绝',
        noPrivileges: '您没有管理员权限。',
        goBack: '返回',
        all: '全部',
        pending: '待处理',
        resolved: '已处理',
        dismissed: '已忽略',
        total: '总计',
        banned: '已封禁',
        reporterNote: '举报人备注：',
        reportedBy: '举报人：{{name}}',
        anonymous: '匿名',
        unknownUser: '未知',
        dismiss: '忽略',
        deletePost: '删除帖子',
        deletePostConfirm: '此操作将永久删除该帖子，确定吗？',
        deletePostDone: '帖子已删除，举报已处理。',
        deletePostFailed: '删除帖子失败。',
        banUser: '封禁用户',
        ban: '封禁',
        banUserConfirm: '要封禁 {{username}} 吗？该用户将无法发布帖子或评论。',
        banUserDone: '已封禁 {{username}}。',
        banUserFailed: '封禁用户失败。',
        dismissFailed: '忽略举报失败。',
        noReports: '未找到举报',
        allCaughtUp: '全部处理完毕！没有待处理的举报。',
        noMatchingReports: '没有符合此筛选条件的举报。',
    },

    // Modal screen
    modal: {
        title: '模态框',
        heading: '这是一个模态框',
        goHome: '前往主屏幕',
    },
};
