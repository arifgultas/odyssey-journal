/**
 * Spanish (Español) translations
 */

export default {
    common: {
        loading: 'Cargando...',
        error: 'Error',
        success: 'Éxito',
        cancel: 'Cancelar',
        save: 'Guardar',
        delete: 'Eliminar',
        edit: 'Editar',
        done: 'Listo',
        next: 'Siguiente',
        back: 'Atrás',
        skip: 'Omitir',
        search: 'Buscar',
        retry: 'Reintentar',
        close: 'Cerrar',
        confirm: 'Confirmar',
        yes: 'Sí',
        no: 'No',
        and: 'y',
        change: 'Cambiar',
        untitled: 'Sin título',
        yesterday: 'Ayer',
        info: 'Información',
        completed: 'Completado',
        progress: 'Progreso',
        unknownLocation: 'Ubicación Desconocida',
        permissionRequired: 'Permiso necesario',
        offlineMode: 'Modo sin conexión',
        pullToRefresh: 'Desliza para actualizar',
        releaseToRefresh: 'Suelta para actualizar',
        percentValue: '{{value}} %',
        ok: 'Aceptar',
    },
    auth: {
        login: 'Iniciar Sesión',
        signup: 'Registrarse',
        logout: 'Cerrar Sesión',
        email: 'Correo electrónico',
        password: 'Contraseña',
        confirmPassword: 'Confirmar contraseña',
        forgotPassword: '¿Olvidaste tu contraseña?',
        resetPassword: 'Restablecer contraseña',
        welcomeBack: '¡Bienvenido de nuevo!',
        createAccount: 'Crear cuenta',
        alreadyHaveAccount: '¿Ya tienes una cuenta?',
        dontHaveAccount: '¿No tienes una cuenta?',
        orContinueWith: 'o continuar con',
        orSignupWith: 'o registrarse con',
        loginSuccess: 'Inicio de sesión exitoso',
        signupSuccess: 'Cuenta creada exitosamente',
        logoutSuccess: 'Sesión cerrada',
        invalidCredentials: 'Correo o contraseña inválidos',
        emailRequired: 'El correo es requerido',
        passwordRequired: 'La contraseña es requerida',
        passwordTooShort: 'La contraseña debe tener al menos 6 caracteres',
        tagline: 'Explora el mundo, comparte tu historia.',
        startJourney: 'Comienza tu viaje hoy.',
        emailOrUsername: 'Correo o nombre de usuario',
        emailAddress: 'Correo electrónico',
        fullName: 'Nombre completo',
        passwordMinChars: 'Contraseña (mínimo 6 caracteres)',
        enterEmailPassword: 'Por favor ingresa tu correo y contraseña.',
        fillAllFields: 'Por favor completa todos los campos.',
        passwordsDontMatch: 'Las contraseñas no coinciden.',
        passwordMinLength: 'La contraseña debe tener al menos 6 caracteres.',
        acceptTermsRequired: 'Debes aceptar los términos para continuar.',
        loginError: 'Error de inicio de sesión',
        signupError: 'Error de registro',
        accountCreated: '¡Cuenta creada!',
        checkEmailVerify: 'Revisa tu correo para verificar tu cuenta.',
        comingSoon: 'Próximamente',
        googleLoginSoon: '¡El inicio con Google estará disponible pronto!',
        appleLoginSoon: '¡El inicio con Apple estará disponible pronto!',
        googleSignupSoon: '¡El registro con Google estará disponible pronto!',
        appleSignupSoon: '¡El registro con Apple estará disponible pronto!',
        termsOfService: 'Términos de servicio',
        privacyPolicy: 'Política de privacidad',
        acceptSuffix: '.',
        copyright: '© 2026 Odyssey Journal. Todos los derechos reservados.',
        forgotPasswordTitle: 'Olvidé mi contraseña',
        forgotPasswordSubtitle: 'No te preocupes, le pasa a todos. Ingresa tu correo para restablecer tu contraseña.',
        enterEmail: 'Por favor ingresa tu correo electrónico.',
        invalidEmail: 'Por favor ingresa un correo válido.',
        emailSent: '¡Correo enviado!',
        resetLinkSentTo: 'El enlace de restablecimiento fue enviado a:',
        checkInboxSpam: 'Revisa tu bandeja de entrada. No olvides revisar la carpeta de spam.',
        backToLogin: 'Volver al inicio de sesión',
        resendEmail: '¿No recibiste el correo? Reenviar',
        sendResetLink: 'Enviar enlace de restablecimiento',
        rememberPassword: '¿Recuerdas tu contraseña?',
        resetLinkValidity: 'Por tu seguridad, el enlace de restablecimiento es válido por 1 hora.',
        newPassword: 'Nueva Contraseña',
        enterNewPassword: 'Introduce tu nueva contraseña',
        confirmNewPassword: 'Confirmar Nueva Contraseña',
        confirmNewPasswordPlaceholder: 'Vuelve a introducir tu nueva contraseña',
        passwordLengthError: 'La contraseña debe tener al menos 6 caracteres.',
        passwordMatchError: 'Las contraseñas no coinciden.',
    },
    onboarding: {
        step1Title: 'Empieza a Explorar',
        step1Desc: 'Descubre las rutas más fascinantes del mundo, obtén consejos de viaje y comparte tus recuerdos inolvidables.',
        step2Title: 'Comparte Tus\nRecuerdos',
        step2Desc: 'Comparte fotos e historias de tus viajes. Conéctate con otros viajeros.',
        step3Title: 'Inspírate\ne Inspira',
        step3Desc: 'Inspírate con nuestra comunidad e inspira a otros con tus propias aventuras.',
        skip: 'Omitir',
        start: 'Comenzar',
        next: 'Siguiente',
    },
    home: {
        title: 'Odyssey Journal',
        noAdventures: 'Sin Aventuras Aún',
        startSharing: '¡Empieza a compartir tus recuerdos de viaje!',
        createFirstPost: 'Crea tu Primera Publicación',
        refreshing: 'Actualizando...',
        publicFeed: 'Público',
        followingFeed: 'Siguiendo',
        noFollowingPosts: 'Sin publicaciones aún',
        exploreTravelers: 'Explora a otros viajeros en la comunidad y síguelos para ver sus publicaciones aquí.',
        exploreButton: 'Explorar viajeros',
        sortLatest: 'Recientes',
        sortPopular: 'Populares',
        sortLocation: 'Con Ubicación',
        sortLatestAccessibility: 'Ordenar por publicaciones más recientes',
        sortPopularAccessibility: 'Ordenar por publicaciones más populares',
        sortLocationAccessibility: 'Filtrar por publicaciones con ubicación',
    },
    explore: {
        title: 'Explorar',
        searchPlaceholder: 'Buscar lugares, usuarios o etiquetas...',
        trending: 'Tendencias',
        categories: 'Categorías',
        users: 'Usuarios',
        places: 'Lugares',
        noResults: 'Sin resultados',
        tryDifferent: 'Intenta con otro término de búsqueda',
        popularPosts: 'Publicaciones Populares',
        viewAll: 'Ver Todo',
        popularDestinations: 'Destinos Populares',
        routes: 'Rutas',
        weeklyDiscovery: 'Descubrimiento Semanal',
        editorsChoice: 'Elección del Editor',
        suggestedTravelers: 'Viajeros Sugeridos',
        follow: 'Seguir',
        loading: 'Cargando...',
        loadFailed: 'Error de carga',
        noSuggestionsYet: 'Sin sugerencias',
        recentSearches: 'Búsquedas Recientes',
        locations: 'Ubicaciones',
        posts: 'Publicaciones',
        discoverRoutes: 'Descubrir Rutas...',
        searchCities: 'Buscar Ciudades...',
        findTravelers: 'Buscar Viajeros...',
        discoverPlaces: 'Descubrir Lugares...',
        unknown: 'Desconocido',
        untitledPost: 'Publicación sin título',
        user: 'Usuario',
        category: 'Categoría',
        noCategoryPosts: 'No hay publicaciones en esta categoría',
        readPost: 'Leer publicación',
        newRoutesToExplore: 'Nuevas rutas por explorar',
        featuredContentDefault: 'Descubre las rutas más hermosas y embárcate en una aventura.',
        viewCategoryPosts: 'Ver publicaciones en la categoría {{category}}',
        featuredDefaultTitle: 'Nuevas Rutas por Explorar',
        featuredDefaultDescription: 'Descubre las rutas más bellas y embárcate en aventuras.',
        postCount: '{{count}} publicaciones',
    },
    create: {
        title: 'Nueva Publicación',
        newEntry: 'Nueva Entrada de Diario',
        whatsOnMind: '¿Qué tienes en mente?',
        addPhoto: 'Agregar Foto',
        addLocation: 'Agregar Ubicación',
        post: 'Publicar',
        posting: 'Publicando...',
        postSuccess: 'Publicación compartida exitosamente',
        postError: 'Error al compartir publicación',
        titleRequired: 'Por favor ingresa un título',
        contentRequired: 'Por favor escribe tu historia',
        chooseOption: 'Elige una opción',
        takePhoto: 'Tomar Foto',
        pickFromGallery: 'Elegir de Galería',
        discardChanges: '¿Descartar cambios?',
        addNote: 'Agregar una nota...',
        confirmLocation: 'Confirmar Ubicación',
        changeLocation: 'Cambiar',
        sealingMemories: 'Sellando tus recuerdos...',
        unknownLocation: 'Ubicación Desconocida',
        discardMessage: 'Se perderán todos los cambios de esta publicación.',
        keepEditing: 'Seguir Editando',
        discard: 'Descartar',
        selectImages: 'Seleccionar Fotos',
        maxImages: 'Puedes seleccionar hasta {{count}} fotos',
        writeCaption: 'Escribe una descripción...',
        categories: 'Categorías',
        images: 'IMÁGENES',
        location: 'UBICACIÓN',
        locationSelected: 'Ubicación seleccionada',
        tapToAddLocation: 'Toca para agregar ubicación',
        titlePlaceholder: 'Título',
        storyPlaceholder: 'Comienza a contar tu historia...',
        emailNotVerifiedTitle: 'Verificación de correo electrónico requerida',
        emailNotVerifiedDesc: 'Por favor, verifica tu dirección de correo electrónico para compartir nuevas entradas con la comunidad.',
        searchPlaceholder: 'Buscar ciudad o lugar...',
        useCurrentLocation: 'Usar ubicación actual (GPS)',
        noResults: 'No se encontraron resultados. Por favor, inténtelo de nuevo.',
        locationNotFound: 'Ubicación no encontrada. Intenta otra búsqueda.',
        locationSearchError: 'Ocurrió un error al buscar la ubicación.',
    },
    categories: {
        nature: 'Naturaleza',
        city: 'Ciudad',
        food: 'Comida',
        history: 'Historia',
        art: 'Arte',
        adventure: 'Aventura',
        beach: 'Playa',
        culture: 'Cultura',
        mountain: 'Montaña',
        wildlife: 'Vida Silvestre',
    },
    collection: {
        saveToCollection: 'Guardar en colección',
        myCollections: 'Mis colecciones',
        nameInputPlaceholder: 'Nombre de la colección...',

        createTitle: 'Crear Colección',
        editTitle: 'Editar Colección',
        save: 'Guardar',
        coverPhoto: 'Foto de Portada',
        addPhoto: 'Agregar Foto',
        collectionName: 'Nombre de la Colección',
        namePlaceholder: 'Ej: Verano 2024',
        themeColor: 'Color del Tema',
        preview: 'Vista Previa',
        errorTitle: 'Error',
        nameRequired: 'Por favor ingresa el nombre de la colección',
        updateError: 'No se pudo actualizar la colección',
        createError: 'No se pudo crear la colección',
        permissionRequired: 'Permiso Requerido',
        galleryPermission: 'Se requiere acceso a la galería',
        zeroPosts: '0 Publicaciones',
        deleteTitle: 'Eliminar Colección',
        deleteConfirm: '¿Estás seguro de que deseas eliminar "{{name}}"? Las publicaciones guardadas no se eliminarán.',
        deleteError: 'No se pudo eliminar la colección',
        loadError: 'No se pudo cargar la colección',
        empty: 'Colección Vacía',
        emptyDesc: 'Aún no se han agregado publicaciones a esta colección',
        notFound: 'Colección no encontrada',
        posts: 'Publicación',
        details: 'Detalles',
        noNote: 'No se ha agregado nota para esta publicación.',
        unknownLocation: 'Ubicación Desconocida',
        untitled: 'Sin Título',
        noNotes: 'No se agregó ninguna nota a esta publicación.',
        saveWithoutCollection: 'Guardar sin colección',
        saveError: 'Error al guardar en la colección',
        create: 'Crear',
    },

    // Chat & Messaging
    chat: {
        title: 'Cartas',
        noConversations: 'Tu bandeja de entrada está vacía',
        startConversation: 'Escribe tu primera carta presionando "Enviar mensaje" en el perfil de cualquier viajero.',
        loading: 'Cargando mensajes...',
        typeMessage: 'Escribe un mensaje...',
        emptyChat: 'Envía tu primer mensaje',
        error: 'Error',
        requests: 'Solicitudes',
        primaryInbox: 'Cartas',
        accept: 'Aceptar',
        decline: 'Eliminar',
        requestBanner: '¿Quieres chatear con este usuario? No puedes responder hasta que aceptes.',
        declineConfirm: '¿Estás seguro de que quieres rechazar esta solicitud? Se eliminará el historial de chat.',
        noRequests: 'No se encontraron solicitudes de mensajes.',
        openLettersAndChats: 'Abrir cartas y chats',
        typing: 'está escribiendo...',
        yesterday: 'Ayer',
    },

    saved: {
        title: 'Guardados',
        noSaved: 'No hay publicaciones guardadas',
        saveDescription: 'Guarda publicaciones que te gusten para verlas después',
        browseExplore: 'Explorar',
        loadError: 'No se pudieron cargar las publicaciones guardadas',
        posts: 'Publicación',
        unknownLocation: 'Ubicación desconocida',
        untitled: 'Sin título',
        details: 'Detalles',
        noNote: 'No hay nota para esta publicación.',
        collections: 'Colecciones',
        new: 'Nuevo',
        noCollectionsYet: 'Aún no hay colecciones',
        createCollectionHint: 'Crea una colección para organizar tus publicaciones',
        noSavedPosts: 'No hay publicaciones guardadas',
        savedPostsWillAppear: 'Tus publicaciones guardadas aparecerán aquí',
    },
    profile: {
        title: 'Perfil',
        editProfile: 'Editar Perfil',
        followers: 'Seguidores',
        following: 'Siguiendo',
        posts: 'Publicaciones',
        settings: 'Configuración',
        myJourney: 'Mi Viaje',
        countries: 'Países',
        kilometers: 'Kilómetros',
        days: 'Días',
        badges: 'Insignias',
        journalNotes: 'Notas del Diario',
        noPostsYet: 'Sin publicaciones aún',
        startJourney: 'Comienza tu viaje',
        logoutTitle: 'Cerrar Sesión',
        logoutMessage: '¿Estás seguro de que deseas cerrar sesión?',
        cancel: 'Cancelar',
        logout: 'Cerrar Sesión',
        journalNote: 'Nota del Diario',
        republicOf: 'República de',
        noJournalNotes: 'Aún no hay notas del diario',
        shareMemories: 'Comparte tus recuerdos de viaje con fotos',
        addFirstNote: 'Agregar Primera Nota',
        travelMap: 'Mapa de Viajes',
        noTravelLocation: 'Aún no hay ubicaciones de viaje',
        shareLocationPosts: 'Comparte publicaciones con ubicación',
        passport: 'PASAPORTE',
        approvedTraveler: 'Viajero Aprobado',
        message: 'Mensaje',
        commonDestinations: 'Destinos Comunes',
        commonPlaces: 'Lugares Comunes',
        noCommonDestinations: 'Aún no hay destinos comunes',
        noCommonDestinationsSubtext: '¡Sigan explorando el mundo!',
        openMap: 'Abrir Mapa',
        travelJournal: 'Diario de Viaje',
        defaultUser: 'Viajero',
        surnameTitle: 'Apellido',
        nameTitle: 'Nombre',
        nationalityTitle: 'Nacionalidad',
        blockUserTitle: 'Bloquear usuario',
        blockUserDesc: '¿Estás seguro de que quieres bloquear a este usuario? No podrá interactuar contigo y sus publicaciones se ocultarán de tu feed.',
        block: 'Bloquear',
        blockSuccess: 'Usuario bloqueado con éxito.',
        blockError: 'Error al bloquear al usuario.',
        notFoundTitle: 'Viajero no encontrado',
        notFoundDesc: 'Este perfil no está disponible o no tienes permiso para verlo.',
        natureLover: 'Amante de la naturaleza',
        cityExplorer: 'Explorador urbano',
        historian: 'Historiador',
        photographer: 'Fotógrafo',
        traveler: 'Viajero',
    },
    editProfile: {
        title: 'Editar Perfil',
        cancel: 'Cancelar',
        save: 'Guardar',
        fullName: 'Nombre Completo',
        fullNamePlaceholder: 'Ingresa tu nombre completo',
        username: 'Nombre de Usuario',
        usernamePlaceholder: 'Ingresa nombre de usuario',
        bio: 'Biografía',
        bioPlaceholder: 'Cuéntanos sobre ti',
        website: 'Sitio Web',
        websitePlaceholder: 'https://tusitio.com',
        changePhoto: 'Cambiar Foto de Perfil',
        saveChanges: 'Guardar Cambios',
        saving: 'Guardando...',
        saved: 'Guardado',
        updateSuccess: 'Perfil actualizado con éxito',
        updateError: 'Error al actualizar perfil',
        permissionNeeded: 'Permiso necesario',
        permissionMessage: 'Concede permiso para acceder a tus fotos',
    },
    notifications: {
        title: 'Notificaciones',
        noNotifications: 'Sin notificaciones',
        checkBack: 'Vuelve a revisar más tarde',
        today: 'Hoy',
        thisWeek: 'Esta Semana',
        older: 'Anteriores',
        markAllRead: 'Marcar todo como leído',
        likedPost: "le gustó '{{postTitle}}'",
        commented: 'comentó en {{postTitle}}',
        followed: 'comenzó a seguirte',
        mentioned: 'te mencionó',
        inboxUpToDate: 'Bandeja de entrada actualizada',
        loadError: 'No se pudieron cargar las notificaciones',
        markAllError: 'No se pudo marcar todo como leído',
        newNotification: 'Nueva notificación',
        nearbyRoute: 'Nueva ruta descubierta cerca: {{locationName}}',
        someone: 'Alguien',
    },
    comments: {
        title: 'Comentarios',
        noComments: 'Sin comentarios',
        beFirst: '¡Sé el primero en comentar!',
        addComment: 'Agregar un comentario...',
        posting: 'Publicando...',
        deleteComment: 'Eliminar Comentario',
        deleteConfirm: '¿Estás seguro de eliminar este comentario?',
        loadError: 'No se pudieron cargar los comentarios',
        addError: 'No se pudo agregar el comentario',
        deleteError: 'No se pudo eliminar el comentario',
    },
    post: {
        like: 'Me gusta',
        unlike: 'Ya no me gusta',
        comment: 'Comentar',
        share: 'Compartir',
        save: 'Guardar',
        unsave: 'Quitar guardado',
        delete: 'Eliminar',
        report: 'Reportar',
        deleteConfirm: '¿Estás seguro de eliminar esta publicación?',
        deleteSuccess: 'Publicación eliminada',
        reportSuccess: 'Reporte enviado',
        memoriesTitle: 'Recuerdos y Notas',
        notesCount: '{{count}} Notas',
        noNotes: 'Aún no hay notas',
        beFirst: '¡Sé el primero en agregar un recuerdo!',
        viewNotes: 'Ver {{count}} notas',
        writeNote: 'Escribe una nota para tu diario...',
        memory: 'Recuerdo',
        deleteTitle: 'Eliminar Publicación',
        deleteError: 'Error al eliminar publicación',
        reportPost: 'Reportar Publicación',
        location: 'Ubicación',
        address: 'Dirección',
        editTitle: 'Editar Publicación',
        boardingPass: 'Tarjeta de embarque',
    },
    report: {
        title: 'Reportar publicación',
        whyReporting: '¿Por qué reportas esta publicación?',
        additionalDetails: 'Detalles adicionales (opcional)',
        detailsPlaceholder: 'Proporcionar más contexto...',
        submit: 'Enviar reporte',
        success: 'Reporte recibido',
        reasons: {
            spam: { label: 'Spam', desc: 'Contenido repetitivo o irrelevante' },
            harassment: { label: 'Acoso o intimidación', desc: 'Atacar o acosar a alguien' },
            hate_speech: { label: 'Discurso de odio', desc: 'Contenido discriminatorio u ofensivo' },
            violence: { label: 'Violencia o amenazas', desc: 'Promover o amenazar con violencia' },
            nudity: { label: 'Desnudez o contenido sexual', desc: 'Contenido sexual inapropiado' },
            false_information: { label: 'Información falsa', desc: 'Contenido engañoso o falso' },
            other: { label: 'Otro', desc: 'Otra cosa' },
        },
        alreadyReportedTitle: 'Ya denunciado',
        alreadyReportedDesc: 'Ya has denunciado esta publicación. La revisaremos pronto.',
        submittedTitle: 'Denuncia enviada',
        submittedDesc: 'Gracias por tu denuncia. La revisaremos y tomaremos las medidas necesarias.',
        submitFailed: 'No se pudo enviar la denuncia. Inténtalo de nuevo.',
    },
    follow: {
        follow: 'Seguir',
        following: 'Siguiendo',
        unfollow: 'Dejar de Seguir',
        followers: 'Seguidores',
        followingList: 'Siguiendo',
        noFollowers: 'Sin seguidores aún',
        noFollowing: 'No sigue a nadie aún',
    },
    settings: {
        title: 'Configuración',
        account: 'Cuenta',
        notifications: 'Notificaciones',
        privacy: 'Privacidad',
        appearance: 'Apariencia',
        language: 'Idioma',
        languageOptions: 'Opciones de idioma',
        darkMode: 'Modo Oscuro',
        about: 'Acerca de',
        help: 'Ayuda',
        terms: 'Términos de Servicio',
        privacyPolicy: 'Política de Privacidad',
        version: 'Versión',
        logout: 'Cerrar Sesión',
        deleteAccount: 'Eliminar Cuenta',
        deleteAccountConfirm: '¿Estás seguro de que deseas eliminar tu cuenta? Esta acción no se puede deshacer.\n\nSi antes quieres una copia de tus datos, escribe a privacy@odysseyjournal.app y la exportaremos por ti.',
        deleteAccountWarning: 'Todas tus publicaciones, comentarios y datos se eliminarán permanentemente.',
        blockedUsers: 'Usuarios bloqueados',
        blockedUsersDesc: 'Administra y desbloquea los usuarios que has bloqueado',
        noBlockedUsers: 'No hay usuarios bloqueados.',
        unblockConfirmTitle: 'Desbloquear usuario',
        unblockConfirmDesc: '¿Estás seguro de que quieres desbloquear a este usuario?',
        unblockSuccess: 'El usuario ha sido desbloqueado.',
        deleteAccountButton: 'Eliminar Mi Cuenta',
        accountDeleted: 'Tu cuenta ha sido eliminada.',
        logoutConfirm: '¿Estás seguro de que quieres cerrar sesión?',
        passportHolder: 'Titular del Pasaporte',
        theme: 'Tema',
        lightDark: 'Claro / Oscuro',
        exitLabel: 'SALIR',
        exportDataTitle: 'Solicitar mis datos',
        exportDataDesc: 'Para obtener una copia de tus datos personales, escribe a privacy@odysseyjournal.app. Exportaremos tu perfil, publicaciones, comentarios, colecciones y conexiones y te los enviaremos.',
        exportDataSubdesc: 'Escríbenos para obtener una copia de tus datos',
        sendEmail: 'Enviar correo',
        legal: 'LEGAL Y COMUNIDAD',
        communityGuidelines: 'Normas de la comunidad',
        communityGuidelinesDesc: 'Reglas para mantener segura a nuestra comunidad',
        adminTitle: 'ADMIN',
        moderationPanel: 'Panel de Moderación',
        moderationPanelDesc: 'Revisar reportes, gestionar usuarios',
        likeNotifications: 'Me gusta',
        likeNotificationsDesc: 'Recibe notificaciones cuando a alguien le guste tu publicación',
        commentNotifications: 'Comentarios',
        commentNotificationsDesc: 'Recibe notificaciones cuando alguien comente tu publicación',
        followNotifications: 'Seguidores',
        followNotificationsDesc: 'Recibe notificaciones cuando alguien empiece a seguirte',
        changePassword: 'Cambiar contraseña',
        changePasswordDesc: 'Actualiza la contraseña de tu cuenta',
        preferencesUpdateError: 'Ocurrió un error al actualizar las preferencias.',
        notificationPreferences: 'Preferencias de Notificación',
        passwordChangeSuccess: 'Tu contraseña se ha actualizado correctamente.',
        unblockError: 'Error al desbloquear al usuario.',
        build: 'Compilación',
        restartTitle: 'Es necesario reiniciar',
        restartMessage: 'Cierra Odyssey Journal y vuelve a abrirlo para completar el cambio de dirección del diseño.',
    },
    errors: {
        generic: 'Algo salió mal',
        network: 'Error de conexión',
        tryAgain: 'Por favor intenta de nuevo',
        sessionExpired: 'Sesión expirada',
        notFound: 'No encontrado',
        unauthorized: 'Acceso no autorizado',
        serverError: 'Error del servidor',
        boundaryTitle: '¡Nuestra brújula se ha perdido!',
        boundarySubtitle: 'Ha ocurrido un error inesperado. Estamos trabajando para solucionarlo.',
        boundaryErrorDetails: 'Detalles del error',
        boundaryRetry: 'Reiniciar aplicación',
        boundaryShowDetails: 'Mostrar detalles del error',
        boundaryHideDetails: 'Ocultar detalles del error',
        noConnection: 'Sin conexión a Internet',
        permissionPhotos: 'Lo sentimos, necesitamos permiso para acceder a tu galería y subir imágenes.',
        permissionCamera: 'Lo sentimos, necesitamos permiso de cámara para tomar fotos.',
        permissionLocation: 'Lo sentimos, necesitamos permiso de ubicación para añadir la ubicación a tus publicaciones.',
        imagePickFailed: 'No se pudo seleccionar la imagen. Inténtalo de nuevo.',
        imagesPickFailed: 'No se pudieron seleccionar las imágenes. Inténtalo de nuevo.',
        photoCaptureFailed: 'No se pudo tomar la foto. Inténtalo de nuevo.',
        locationFailed: 'No se pudo obtener la ubicación actual. Inténtalo de nuevo.',
        signInTitle: 'Error de inicio de sesión',
        signInProviderFailed: 'Hubo un problema al iniciar sesión con {{provider}}.',
        shareFailed: 'No se pudo compartir la publicación',
        emailNotConfirmed: 'Confirma tu dirección de correo antes de iniciar sesión',
        emailExists: 'Ya existe una cuenta con este correo electrónico',
        weakPassword: 'Elige una contraseña más segura',
        samePassword: 'La nueva contraseña debe ser distinta de la actual',
        rateLimit: 'Demasiados intentos. Espera un momento e inténtalo de nuevo',
        linkExpired: 'Este enlace ha caducado. Solicita uno nuevo',
        userNotFound: 'No se encontró ninguna cuenta con este correo electrónico',
        signupDisabled: 'Los nuevos registros están cerrados por ahora',
        usernameTaken: 'Este nombre de usuario ya está en uso',
    },
    time: {
        now: 'Ahora',
        minuteAgo: 'hace {{count}} minuto',
        minutesAgo: 'hace {{count}} minutos',
        hourAgo: 'hace {{count}} hora',
        hoursAgo: 'hace {{count}} horas',
        dayAgo: 'hace {{count}} día',
        daysAgo: 'hace {{count}} días',
        weekAgo: 'hace {{count}} semana',
        weeksAgo: 'hace {{count}} semanas',
        monthAgo: 'hace {{count}} mes',
        monthsAgo: 'hace {{count}} meses',
        yearAgo: 'hace {{count}} año',
        yearsAgo: 'hace {{count}} años',
        yesterday: 'Ayer',
        lastWeek: 'Semana pasada',
    },
    empty: {
        noData: 'Sin datos',
        noPosts: 'Sin publicaciones',
        noUsers: 'Sin usuarios',
        noPlaces: 'Sin lugares',
    },
    map: {
        title: 'Mapa',
        loading: 'Cargando mapa...',
        notSupportedWeb: 'Mapa no disponible en Web',
        notSupportedExpo: 'Mapa no disponible en Expo Go',
        useMobileApp: 'Usa la app móvil para ver el mapa.',
        useDevBuild: 'Se requiere Development Build.',
        fallbackList: 'Puedes ver la lista de lugares abajo.',
        stats: '{{locationCount}} lugares • {{postCount}} publicaciones',
        unknownLocation: 'Ubicación desconocida',
        postCount: '{{count}} publicaciones',
        noPosts: 'No hay publicaciones con ubicación',
        addLocationTip: 'Agrega ubicación a tus publicaciones para verlas aquí',
        openInMap: 'Abrir en mapa',
        travelPoint: 'Punto de viaje',
        journeyRoute: 'Ruta de Viaje ({{count}} Puntos)',
        noJourneyYet: 'Aún no hay viajes',
        startExploring: '¡Empieza a explorar el mundo!',
        notSupported: 'Mapa no compatible',
        locationVisited: 'Lugar visitado',
        locationsVisited: 'Lugares visitados',
        appleMaps: 'Apple Maps',
        openInMaps: 'Mapas',
    },

    // Destination Posts
    destinationPosts: {
        empty: 'No hay publicaciones en esta ubicación',
        emptyDesc: 'Aún no se han compartido publicaciones en este destino',
        destination: 'Destino',
        untitledPost: 'Publicación sin título',
        user: 'Usuario',
        error: 'Ocurrió un error',
    },

    // Followers/Following
    followers: {
        title: 'Seguidores',
        noFollowers: 'Aún no hay seguidores',
        noFollowersDesc: 'Cuando las personas sigan a este usuario, aparecerán aquí',
        loadError: 'Error al cargar seguidores',
        followError: 'Error al actualizar el estado de seguimiento',
    },

    following: {
        title: 'Siguiendo',
        notFollowing: 'No sigue a nadie',
        notFollowingDesc: 'Cuando este usuario siga a personas, aparecerán aquí',
        loadError: 'Error al cargar la lista de seguidos',
        followError: 'Error al actualizar el estado de seguimiento',
    },

    // Popular Posts
    popularPosts: {
        title: 'Publicaciones populares',
        empty: 'No hay publicaciones populares',
        emptyDesc: 'Aún no se han encontrado publicaciones populares',
        untitledPost: 'Publicación sin título',
        user: 'Usuario',
        error: 'Ocurrió un error',
    },

    // Create Post (additional keys)
    createPost: {
        newEntry: 'Nueva entrada de diario',
        cancel: 'Cancelar',
        share: 'Compartir',
        images: 'IMÁGENES',
        addPhoto: 'Añadir foto',
        addNote: 'Añadir nota...',
        confirmLocation: 'Confirmar ubicación',
        changeLocation: 'Cambiar',
        titlePlaceholder: 'Título',
        storyPlaceholder: 'Comienza a contar tu historia...',
        errorTitle: 'Error',
        enterTitle: 'Por favor ingresa un título',
        enterStory: 'Por favor escribe tu historia',
        success: 'Éxito',
        entryCreated: '¡Tu entrada de diario ha sido creada!',
        ok: 'OK',
        createError: 'No se pudo crear la publicación. Por favor intenta de nuevo.',
        addPhotoTitle: 'Añadir foto',
        chooseOption: 'Elige una opción',
        takePhoto: 'Tomar foto',
        pickFromGallery: 'Elegir de la galería',
        nature: 'Naturaleza',
        city: 'Ciudad',
        food: 'Comida',
        history: 'Historia',
        art: 'Arte',
        emailNotVerifiedTitle: 'Verificación de correo electrónico requerida',
        emailNotVerifiedDesc: 'Por favor, verifica tu dirección de correo electrónico para compartir nuevas entradas con la comunidad.',
    },


    weather: {
        clear: 'Despejado',
        mainlyClear: 'Mayormente Despejado',
        partlyCloudy: 'Parcialmente Nublado',
        overcast: 'Nublado',
        foggy: 'Niebla',
        depositingRimeFog: 'Niebla con Escarcha',
        lightDrizzle: 'Llovizna Ligera',
        moderateDrizzle: 'Llovizna Moderada',
        denseDrizzle: 'Llovizna Densa',
        freezingDrizzle: 'Llovizna Helada',
        denseFreezingDrizzle: 'Llovizna Helada Densa',
        slightRain: 'Lluvia Ligera',
        moderateRain: 'Lluvia Moderada',
        heavyRain: 'Lluvia Fuerte',
        freezingRain: 'Lluvia Helada',
        heavyFreezingRain: 'Lluvia Helada Fuerte',
        slightSnow: 'Nevada Ligera',
        moderateSnow: 'Nevada Moderada',
        heavySnow: 'Nevada Fuerte',
        snowGrains: 'Granizo de Nieve',
        slightRainShowers: 'Chaparrón Ligero',
        moderateRainShowers: 'Chaparrón Moderado',
        violentRainShowers: 'Chaparrón Violento',
        slightSnowShowers: 'Nevada Ligera',
        heavySnowShowers: 'Nevada Fuerte',
        thunderstorm: 'Tormenta',
        thunderstormSlightHail: 'Tormenta con Granizo Ligero',
        thunderstormHeavyHail: 'Tormenta con Granizo Fuerte',
        unknown: 'Desconocido',
    },
    guidelines: {
        intro: '¡Bienvenido a Odyssey Journal! Queremos mantener este espacio seguro, inspirador y centrado en experiencias de viaje genuinas. Sigue estas normas.',
        respectTitle: 'Respeta a la comunidad',
        respectDesc: 'Odyssey Journal es un lugar para que los viajeros compartan sus experiencias reales. Trata a los demás con respeto. El discurso de odio, el acoso, la intimidación y la discriminación de cualquier tipo están estrictamente prohibidos.',
        authenticTitle: 'Comparte viajes auténticos',
        authenticDesc: 'Concéntrate en los viajes, los lugares, la cultura y tu viaje personal. No publiques spam, anuncios, propaganda política ni contenido no relacionado.',
        safeTitle: 'Mantenlo seguro y apropiado',
        safeDesc: 'No publiques desnudos, contenido sexualmente explícito ni violencia gráfica. Asegúrate de que tus fotos y textos sean apropiados para el público en general.',
        privacyTitle: 'Respeta la privacidad',
        privacyDesc: 'No compartas información personal de otros sin su consentimiento explícito. Ten cuidado con a quién y a qué capturas en tus fotos.',
        enforcementTitle: 'Cumplimiento',
        enforcementDesc: 'Revisamos el contenido y las cuentas reportadas. Violar estas normas puede resultar en la eliminación del contenido, acceso restringido o eliminación permanente de la cuenta sin previo aviso.',
        thankYou: '¡Gracias por ayudarnos a hacer de Odyssey Journal un gran lugar para los viajeros!'
    },
    // Content Moderation
    moderation: {
        flaggedNoReasons: 'Tu contenido ha sido marcado para revisión. Asegúrate de que cumpla con nuestras normas comunitarias.',
        flaggedWithReasons: 'Tu contenido ha sido marcado por: {{reasons}}. Asegúrate de que cumpla con nuestras normas comunitarias.',
        categories: {
            harassment: 'acoso',
            'harassment/threatening': 'acoso amenazante',
            hate: 'discurso de odio',
            'hate/threatening': 'odio amenazante',
            'self-harm': 'autolesión',
            'self-harm/intent': 'intento de autolesión',
            'self-harm/instructions': 'instrucciones de autolesión',
            sexual: 'contenido sexual',
            'sexual/minors': 'contenido sexual de menores',
            violence: 'violencia',
            'violence/graphic': 'violencia gráfica'
        }
    },
    homeLocation: {
        title: 'Establece tu ubicación de origen',
        description: 'Define tu ciudad de origen para que podamos calcular con precisión tu distancia total de viaje y las estadísticas de tu pasaporte.',
        useCurrentLocation: 'Usar mi ubicación actual',
        locating: 'Localizando...',
        later: 'Más tarde',
        success: 'Ubicación de origen configurada con éxito.',
        permissionDenied: 'Permiso de ubicación denegado. Puedes configurarlo más tarde en tu perfil.',
        error: 'Error al obtener la ubicación.',
    },
    badgeList: {
        firstAdventure: {
            name: "Primera Aventura",
            description: "Comparte tu primera publicación"
        },
        worldTraveler: {
            name: "Trotamundos",
            description: "Visita 5 países diferentes"
        },
        photographer: {
            name: "Maestro Fotógrafo",
            description: "Comparte 10 publicaciones con fotos"
        },
        gourmet: {
            name: "Gourmet",
            description: "Comparte 3 publicaciones en la categoría comida"
        },
        marathonTraveler: {
            name: "Viajero Maratón",
            description: "Viaja 10.000 km"
        },
        explorer: {
            name: "Explorador",
            description: "Viaja durante 30 días"
        }
    },
    // Date Picker
    datePicker: {
        title: 'FECHA / HORA DEL VIAJE',
        tapToChange: 'Toca para cambiar la fecha',
        change: 'Editar',
        today: 'Hoy',
        yesterday: 'Ayer',
        tomorrow: 'Mañana',
        threeDaysAgo: 'Hace 3 días',
        calendar: 'Calendario',
        selectDate: 'Seleccionar fecha del viaje',
        selected: 'Seleccionado:',
        apply: 'Aplicar fecha',
        cancel: 'Cancelar',
        daysAgo: 'Hace {{count}} días',
        daysLater: 'En {{count}} días',
    },

    // Admin panel
    admin: {
        panelTitle: 'PANEL DE ADMINISTRACIÓN',
        accessDenied: 'Acceso denegado',
        noPrivileges: 'No tienes privilegios de administrador.',
        goBack: 'Volver',
        all: 'Todos',
        pending: 'Pendientes',
        resolved: 'Resueltos',
        dismissed: 'Descartados',
        total: 'Total',
        banned: 'Bloqueados',
        reporterNote: 'Nota del denunciante:',
        reportedBy: 'Denunciado por: {{name}}',
        anonymous: 'Anónimo',
        unknownUser: 'Desconocido',
        dismiss: 'Descartar',
        deletePost: 'Eliminar publicación',
        deletePostConfirm: 'Esto eliminará la publicación de forma permanente. ¿Estás seguro?',
        deletePostDone: 'Publicación eliminada y denuncia resuelta.',
        deletePostFailed: 'No se pudo eliminar la publicación.',
        banUser: 'Bloquear usuario',
        ban: 'Bloquear',
        banUserConfirm: '¿Bloquear a {{username}}? No podrá crear publicaciones ni comentarios.',
        banUserDone: '{{username}} ha sido bloqueado.',
        banUserFailed: 'No se pudo bloquear al usuario.',
        dismissFailed: 'No se pudo descartar la denuncia.',
        noReports: 'No se encontraron denuncias',
        allCaughtUp: '¡Todo al día! No hay denuncias pendientes.',
        noMatchingReports: 'Ninguna denuncia coincide con este filtro.',
    },

    // Modal screen
    modal: {
        title: 'Modal',
        heading: 'Esta es una ventana modal',
        goHome: 'Ir a la pantalla de inicio',
    },
};
