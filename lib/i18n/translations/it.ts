/**
 * Italian (Italiano) translations
 */

export default {
    common: {
        loading: 'Caricamento...',
        error: 'Errore',
        success: 'Successo',
        cancel: 'Annulla',
        save: 'Salva',
        delete: 'Elimina',
        edit: 'Modifica',
        done: 'Fatto',
        next: 'Avanti',
        back: 'Indietro',
        skip: 'Salta',
        search: 'Cerca',
        retry: 'Riprova',
        close: 'Chiudi',
        confirm: 'Conferma',
        yes: 'Sì',
        no: 'No',
        and: 'e',
        change: 'Modifica',
        untitled: 'Senza titolo',
        yesterday: 'Ieri',
        info: 'Informazione',
        completed: 'Completato',
        progress: 'Progresso',
        unknownLocation: 'Posizione Sconosciuta',
        permissionRequired: 'Autorizzazione richiesta',
        offlineMode: 'Modalità offline',
        pullToRefresh: 'Trascina per aggiornare',
        releaseToRefresh: 'Rilascia per aggiornare',
        percentValue: '{{value}}%',
        ok: 'OK',
    },
    auth: {
        login: 'Accedi',
        signup: 'Registrati',
        logout: 'Esci',
        email: 'Email',
        password: 'Password',
        confirmPassword: 'Conferma password',
        forgotPassword: 'Password dimenticata?',
        resetPassword: 'Reimposta password',
        welcomeBack: 'Bentornato!',
        createAccount: 'Crea account',
        alreadyHaveAccount: 'Hai già un account?',
        dontHaveAccount: 'Non hai un account?',
        orContinueWith: 'o continua con',
        orSignupWith: 'o registrati con',
        loginSuccess: 'Accesso effettuato',
        signupSuccess: 'Account creato',
        logoutSuccess: 'Disconnesso',
        invalidCredentials: 'Email o password non validi',
        emailRequired: "L'email è richiesta",
        passwordRequired: 'La password è richiesta',
        passwordTooShort: 'La password deve avere almeno 6 caratteri',
        tagline: 'Esplora il mondo, condividi la tua storia.',
        startJourney: 'Inizia il tuo viaggio oggi.',
        emailOrUsername: 'Email o nome utente',
        emailAddress: 'Indirizzo email',
        fullName: 'Nome completo',
        passwordMinChars: 'Password (minimo 6 caratteri)',
        enterEmailPassword: 'Inserisci email e password.',
        fillAllFields: 'Compila tutti i campi.',
        passwordsDontMatch: 'Le password non corrispondono.',
        passwordMinLength: 'La password deve avere almeno 6 caratteri.',
        acceptTermsRequired: 'Devi accettare i termini.',
        loginError: 'Errore di accesso',
        signupError: 'Errore di registrazione',
        accountCreated: 'Account creato!',
        checkEmailVerify: 'Controlla la tua email per la verifica.',
        comingSoon: 'Prossimamente',
        googleLoginSoon: 'Accesso con Google presto disponibile!',
        appleLoginSoon: 'Accesso con Apple presto disponibile!',
        googleSignupSoon: 'Registrazione con Google presto disponibile!',
        appleSignupSoon: 'Registrazione con Apple presto disponibile!',
        termsOfService: 'Termini di servizio',
        privacyPolicy: 'Informativa sulla privacy',
        acceptSuffix: '.',
        copyright: '© 2026 Odyssey Journal. Tutti i diritti riservati.',
        forgotPasswordTitle: 'Password dimenticata',
        forgotPasswordSubtitle: 'Non preoccuparti, succede. Inserisci la tua email per reimpostare.',
        enterEmail: 'Inserisci il tuo indirizzo email.',
        invalidEmail: 'Inserisci un\'email valida.',
        emailSent: 'Email inviata!',
        resetLinkSentTo: 'Il link di reimpostazione è stato inviato a:',
        checkInboxSpam: 'Controlla la posta in arrivo. Non dimenticare lo spam.',
        backToLogin: 'Torna al login',
        resendEmail: 'Non ricevuta? Reinvia',
        sendResetLink: 'Invia link',
        rememberPassword: 'Ti ricordi?',
        resetLinkValidity: 'Per la tua sicurezza, il link è valido 1 ora.',
        newPassword: 'Nuova Password',
        enterNewPassword: 'Inserisci la tua nuova password',
        confirmNewPassword: 'Conferma Nuova Password',
        confirmNewPasswordPlaceholder: 'Reinserisci la tua nuova password',
        passwordLengthError: 'La password deve contenere almeno 6 caratteri.',
        passwordMatchError: 'Le password non corrispondono.',
    },
    onboarding: {
        step1Title: 'Inizia ad Esplorare',
        step1Desc: 'Scopri i percorsi più affascinanti del mondo, ottieni consigli di viaggio e condividi i tuoi ricordi indimenticabili.',
        step2Title: 'Condividi i Tuoi\nRicordi',
        step2Desc: 'Condividi foto e storie dai tuoi viaggi. Connettiti con altri viaggiatori.',
        step3Title: 'Lasciati Ispirare\ne Ispira',
        step3Desc: 'Lasciati ispirare dalla nostra community e ispira gli altri con le tue avventure.',
        skip: 'Salta',
        start: 'Inizia',
        next: 'Avanti',
    },
    home: {
        title: 'Odyssey Journal',
        noAdventures: 'Nessuna Avventura',
        startSharing: 'Inizia a condividere i tuoi ricordi di viaggio!',
        createFirstPost: 'Crea il tuo Primo Post',
        refreshing: 'Aggiornamento...',
        publicFeed: 'Feed Pubblico',
        followingFeed: 'Seguiti',
        noFollowingPosts: 'Nessun post ancora',
        exploreTravelers: 'Esplora altri viaggiatori nella community e seguili per vedere i loro post qui.',
        exploreButton: 'Esplora viaggiatori',
        sortLatest: 'Recenti',
        sortPopular: 'Popolari',
        sortLocation: 'Con Posizione',
        sortLatestAccessibility: 'Ordina per post più recenti',
        sortPopularAccessibility: 'Ordina per post più popolari',
        sortLocationAccessibility: 'Filtra per post con posizione',
    },
    explore: {
        title: 'Esplora',
        searchPlaceholder: 'Cerca luoghi, utenti o tag...',
        trending: 'Tendenze',
        categories: 'Categorie',
        users: 'Utenti',
        places: 'Luoghi',
        noResults: 'Nessun risultato',
        tryDifferent: 'Prova un altro termine di ricerca',
        popularPosts: 'Post Popolari',
        viewAll: 'Vedi tutto',
        popularDestinations: 'Destinazioni Popolari',
        routes: 'Itinerari',
        weeklyDiscovery: 'Scoperta Settimanale',
        editorsChoice: 'Scelta dell\'editore',
        suggestedTravelers: 'Viaggiatori Suggeriti',
        follow: 'Segui',
        loading: 'Caricamento...',
        loadFailed: 'Caricamento fallito',
        noSuggestionsYet: 'Nessun suggerimento ancora',
        recentSearches: 'Ricerche Recenti',
        locations: 'Luoghi',
        posts: 'Post',
        discoverRoutes: 'Scopri itinerari...',
        searchCities: 'Cerca città...',
        findTravelers: 'Trova viaggiatori...',
        discoverPlaces: 'Scopri nuovi luoghi...',
        unknown: 'Sconosciuto',
        untitledPost: 'Post senza titolo',
        user: 'Utente',
        category: 'Categoria',
        noCategoryPosts: 'Nessun post in questa categoria',
        readPost: 'Leggi il post',
        newRoutesToExplore: 'Nuove rotte da esplorare',
        featuredContentDefault: 'Scopri gli itinerari più suggestivi e parti all\'avventura.',
        viewCategoryPosts: 'Visualizza post nella categoria {{category}}',
        featuredDefaultTitle: 'Nuovi Itinerari da Esplorare',
        featuredDefaultDescription: 'Scopri i percorsi più belli e parti per nuove avventure.',
        postCount: '{{count}} post',
    },
    create: {
        title: 'Nuovo Post',
        newEntry: 'Nuova Voce del Diario',
        whatsOnMind: 'A cosa stai pensando?',
        addPhoto: 'Aggiungi Foto',
        addLocation: 'Aggiungi Luogo',
        post: 'Pubblica',
        posting: 'Pubblicazione...',
        postSuccess: 'Post condiviso con successo',
        postError: 'Errore nella condivisione',
        titleRequired: 'Inserisci un titolo',
        contentRequired: 'Scrivi la tua storia',
        chooseOption: 'Scegli un\'opzione',
        takePhoto: 'Scatta Foto',
        pickFromGallery: 'Scegli dalla Galleria',
        discardChanges: 'Annullare le modifiche?',
        addNote: 'Aggiungi una nota...',
        confirmLocation: 'Conferma Posizione',
        changeLocation: 'Cambia',
        sealingMemories: 'Sigillando i tuoi ricordi...',
        unknownLocation: 'Posizione Sconosciuta',
        discardMessage: 'Tutte le modifiche andranno perse.',
        keepEditing: 'Continua a modificare',
        discard: 'Annulla',
        selectImages: 'Seleziona Foto',
        maxImages: 'Puoi selezionare fino a {{count}} foto',
        writeCaption: 'Scrivi una didascalia...',
        categories: 'Categorie',
        images: 'IMMAGINI',
        location: 'POSIZIONE',
        locationSelected: 'Posizione selezionata',
        tapToAddLocation: 'Tocca per aggiungere posizione',
        titlePlaceholder: 'Titolo',
        storyPlaceholder: 'Inizia a raccontare la tua storia...',
        emailNotVerifiedTitle: 'Verifica email richiesta',
        emailNotVerifiedDesc: 'Verifica il tuo indirizzo email per condividere nuove voci di diario con la community.',
        searchPlaceholder: 'Cerca città o luogo...',
        useCurrentLocation: 'Usa posizione attuale (GPS)',
        noResults: 'Nessun risultato trovato. Riprova.',
        locationNotFound: 'Posizione non trovata. Prova un\'altra ricerca.',
        locationSearchError: 'Si è verificato un errore durante la ricerca della posizione.',
    },
    categories: {
        nature: 'Natura',
        city: 'Città',
        food: 'Cibo',
        history: 'Storia',
        art: 'Arte',
        adventure: 'Avventura',
        beach: 'Spiaggia',
        culture: 'Cultura',
        mountain: 'Montagna',
        wildlife: 'Fauna Selvatica',
    },
    collection: {
        saveToCollection: 'Salva nella raccolta',
        myCollections: 'Le mie raccolte',
        nameInputPlaceholder: 'Nome della raccolta...',

        createTitle: 'Crea Collezione',
        editTitle: 'Modifica Collezione',
        save: 'Salva',
        coverPhoto: 'Foto di Copertina',
        addPhoto: 'Aggiungi Foto',
        collectionName: 'Nome Collezione',
        namePlaceholder: 'Es: Estate 2024',
        themeColor: 'Colore Tema',
        preview: 'Anteprima',
        errorTitle: 'Errore',
        nameRequired: 'Inserisci il nome della collezione',
        updateError: 'Impossibile aggiornare la collezione',
        createError: 'Impossibile creare la collezione',
        permissionRequired: 'Permesso Richiesto',
        galleryPermission: 'È richiesto l\'accesso alla galleria',
        zeroPosts: '0 Post',
        deleteTitle: 'Elimina Collezione',
        deleteConfirm: 'Sei sicuro di voler eliminare "{{name}}"? I post salvati non verranno eliminati.',
        deleteError: 'Impossibile eliminare la collezione',
        loadError: 'Impossibile caricare la collezione',
        empty: 'Collezione Vuota',
        emptyDesc: 'Nessun post aggiunto a questa collezione',
        notFound: 'Collezione non trovata',
        posts: 'Post',
        details: 'Dettagli',
        noNote: 'Nessuna nota aggiunta per questo post.',
        unknownLocation: 'Posizione Sconosciuta',
        untitled: 'Senza Titolo',
        noNotes: 'Nessuna nota aggiunta a questo post.',
        saveWithoutCollection: 'Salva senza collezione',
        saveError: 'Errore durante il salvataggio',
        create: 'Crea',
    },

    // Chat & Messaging
    chat: {
        title: 'Lettere',
        noConversations: 'La tua posta in arrivo è vuota',
        startConversation: 'Scrivi la tua prima lettera premendo "Invia messaggio" sul profilo di qualsiasi viaggiatore.',
        loading: 'Caricamento dei messaggi...',
        typeMessage: 'Scrivi un messaggio...',
        emptyChat: 'Invia il tuo primo messaggio',
        error: 'Errore',
        requests: 'Richieste',
        primaryInbox: 'Lettere',
        accept: 'Accetta',
        decline: 'Elimina',
        requestBanner: 'Vuoi chattare con questo utente? Non puoi rispondere finché non accetti.',
        declineConfirm: 'Sei sicuro di voler rifiutare questa richiesta? La cronologia della chat verrà eliminata.',
        noRequests: 'Nessuna richiesta di messaggio trovata.',
        openLettersAndChats: 'Apri lettere e chat',
        typing: 'sta scrivendo...',
        yesterday: 'Ieri',
    },

    saved: {
        title: 'Salvati',
        noSaved: 'Nessun post salvato',
        saveDescription: 'Salva i post che ti piacciono per vederli dopo',
        browseExplore: 'Sfoglia Esplora',
        loadError: 'Impossibile caricare i post salvati',
        posts: 'Post',
        unknownLocation: 'Posizione sconosciuta',
        untitled: 'Senza titolo',
        details: 'Dettagli',
        noNote: 'Nessuna nota per questo post.',
        collections: 'Collezioni',
        new: 'Nuovo',
        noCollectionsYet: 'Nessuna collezione ancora',
        createCollectionHint: 'Crea una collezione per organizzare',
        noSavedPosts: 'Nessun post salvato',
        savedPostsWillAppear: 'I tuoi post salvati appariranno qui',
    },
    profile: {
        title: 'Profilo',
        editProfile: 'Modifica Profilo',
        followers: 'Seguaci',
        following: 'Seguiti',
        posts: 'Post',
        settings: 'Impostazioni',
        myJourney: 'Il mio Viaggio',
        countries: 'Paesi',
        kilometers: 'Chilometri',
        days: 'Giorni',
        badges: 'Badge',
        journalNotes: 'Note del Diario',
        noPostsYet: 'Nessun post ancora',
        startJourney: 'Inizia il tuo viaggio',
        logoutTitle: 'Esci',
        logoutMessage: 'Sei sicuro di voler uscire?',
        cancel: 'Annulla',
        logout: 'Esci',
        journalNote: 'Nota del Diario',
        republicOf: 'Repubblica di',
        noJournalNotes: 'Ancora nessuna nota del diario',
        shareMemories: 'Condividi i tuoi ricordi di viaggio con foto',
        addFirstNote: 'Aggiungi Prima Nota',
        travelMap: 'Mappa di Viaggio',
        noTravelLocation: 'Ancora nessuna posizione di viaggio',
        shareLocationPosts: 'Condividi post con posizione',
        passport: 'PASSAPORTO',
        approvedTraveler: 'Viaggiatore Approvato',
        message: 'Messaggio',
        commonDestinations: 'Destinazioni comuni',
        commonPlaces: 'Luoghi comuni',
        noCommonDestinations: 'Nessuna destinazione comune ancora',
        noCommonDestinationsSubtext: 'Continuate a esplorare il mondo!',
        openMap: 'Apri Mappa',
        travelJournal: 'Diario di viaggio',
        defaultUser: 'Viaggiatore',
        surnameTitle: 'Cognome',
        nameTitle: 'Nome',
        nationalityTitle: 'Nazionalità',
        blockUserTitle: 'Blocca utente',
        blockUserDesc: 'Sei sicuro di voler bloccare questo utente? Non potrà più interagire con te e i suoi post saranno nascosti nel tuo feed.',
        block: 'Blocca',
        blockSuccess: 'Utente bloccato con successo.',
        blockError: 'Errore durante il blocco utente.',
        notFoundTitle: 'Viaggiatore non trovato',
        notFoundDesc: 'Questo profilo non è disponibile o non hai i permessi per visualizzarlo.',
        natureLover: 'Amante della natura',
        cityExplorer: 'Esploratore urbano',
        historian: 'Storico',
        photographer: 'Fotografo',
        traveler: 'Viaggiatore',
    },
    editProfile: {
        title: 'Modifica Profilo',
        cancel: 'Annulla',
        save: 'Salva',
        fullName: 'Nome Completo',
        fullNamePlaceholder: 'Inserisci il tuo nome completo',
        username: 'Nome Utente',
        usernamePlaceholder: 'Inserisci nome utente',
        bio: 'Bio',
        bioPlaceholder: 'Parlaci di te',
        website: 'Sito Web',
        websitePlaceholder: 'https://tuosito.com',
        changePhoto: 'Cambia Foto Profilo',
        saveChanges: 'Salva Modifiche',
        saving: 'Salvataggio...',
        saved: 'Salvato',
        updateSuccess: 'Profilo aggiornato con successo',
        updateError: "Errore nell'aggiornamento del profilo",
        permissionNeeded: 'Permesso richiesto',
        permissionMessage: 'Concedi il permesso per accedere alle tue foto',
    },
    notifications: {
        title: 'Notifiche',
        noNotifications: 'Nessuna notifica',
        checkBack: 'Controlla più tardi',
        today: 'Oggi',
        thisWeek: 'Questa Settimana',
        older: 'Più vecchie',
        markAllRead: 'Segna tutto come letto',
        likedPost: "ha messo mi piace a '{{postTitle}}'",
        commented: 'ha commentato su {{postTitle}}',
        followed: 'ha iniziato a seguirti',
        mentioned: 'ti ha menzionato',
        inboxUpToDate: 'Posta in arrivo aggiornata',
        loadError: 'Impossibile caricare le notifiche',
        markAllError: 'Impossibile segnare tutto come letto',
        newNotification: 'Nuova notifica',
        nearbyRoute: 'Nuovo percorso scoperto nelle vicinanze: {{locationName}}',
        someone: 'Qualcuno',
    },
    comments: {
        title: 'Commenti',
        noComments: 'Nessun commento',
        beFirst: 'Sii il primo a commentare!',
        addComment: 'Aggiungi un commento...',
        posting: 'Pubblicazione...',
        deleteComment: 'Elimina Commento',
        deleteConfirm: 'Sei sicuro di voler eliminare questo commento?',
        loadError: 'Impossibile caricare i commenti',
        addError: 'Impossibile aggiungere il commento',
        deleteError: 'Impossibile eliminare il commento',
    },
    post: {
        like: 'Mi piace',
        unlike: 'Non mi piace più',
        comment: 'Commenta',
        share: 'Condividi',
        save: 'Salva',
        unsave: 'Rimuovi',
        delete: 'Elimina',
        report: 'Segnala',
        deleteConfirm: 'Sei sicuro di voler eliminare questo post?',
        deleteSuccess: 'Post eliminato',
        reportSuccess: 'Segnalazione inviata',
        memoriesTitle: 'Ricordi e Note',
        notesCount: '{{count}} Note',
        noNotes: 'Ancora nessuna nota',
        beFirst: 'Sii il primo ad aggiungere un ricordo!',
        viewNotes: 'Visualizza {{count}} note',
        writeNote: 'Scrivi una nota...',
        memory: 'Ricordo',
        deleteTitle: 'Elimina Post',
        deleteError: 'Impossibile eliminare il post',
        reportPost: 'Segnala Post',
        location: 'Posizione',
        address: 'Indirizzo',
        editTitle: 'Modifica Post',
        boardingPass: 'Carta d\'imbarco',
    },
    report: {
        title: 'Segnala post',
        whyReporting: 'Perché stai segnalando questo post?',
        additionalDetails: 'Dettagli aggiuntivi (opzionale)',
        detailsPlaceholder: 'Fornire più contesto...',
        submit: 'Invia segnalazione',
        success: 'Segnalazione ricevuta',
        reasons: {
            spam: { label: 'Spam', desc: 'Contenuto ripetitivo o irrilevante' },
            harassment: { label: 'Molestie o bullismo', desc: 'Prendere di mira o attaccare qualcuno' },
            hate_speech: { label: 'Incitamento all\'odio', desc: 'Contenuto discriminatorio o offensivo' },
            violence: { label: 'Violenza o minacce', desc: 'Promuovere o minacciare violenza' },
            nudity: { label: 'Nudità o contenuto sessuale', desc: 'Contenuto sessuale inappropriato' },
            false_information: { label: 'Informazioni false', desc: 'Contenuto fuorviante o falso' },
            other: { label: 'Altro', desc: 'Qualcos\'altro' },
        },
        alreadyReportedTitle: 'Già segnalato',
        alreadyReportedDesc: 'Hai già segnalato questo post. Lo esamineremo a breve.',
        submittedTitle: 'Segnalazione inviata',
        submittedDesc: 'Grazie per la segnalazione. La esamineremo e prenderemo i provvedimenti necessari.',
        submitFailed: 'Impossibile inviare la segnalazione. Riprova.',
    },
    follow: {
        follow: 'Segui',
        following: 'Seguiti',
        unfollow: 'Smetti di seguire',
        followers: 'Seguaci',
        followingList: 'Seguiti',
        noFollowers: 'Ancora nessun seguace',
        noFollowing: 'Non segue nessuno',
    },
    settings: {
        title: 'Impostazioni',
        account: 'Account',
        notifications: 'Notifiche',
        privacy: 'Privacy',
        appearance: 'Aspetto',
        language: 'Lingua',
        languageOptions: 'Opzioni lingua',
        darkMode: 'Modalità Scura',
        about: 'Info',
        help: 'Aiuto',
        terms: 'Termini di Servizio',
        privacyPolicy: 'Informativa sulla Privacy',
        version: 'Versione',
        logout: 'Esci',
        deleteAccount: 'Elimina Account',
        deleteAccountConfirm: 'Sei sicuro di voler eliminare il tuo account? Questa azione non può essere annullata.\n\nSe prima desideri una copia dei tuoi dati, scrivi a privacy@odysseyjournal.app e la esporteremo per te.',
        deleteAccountWarning: 'Tutti i tuoi post, commenti e dati verranno eliminati definitivamente.',
        blockedUsers: 'Utenti bloccati',
        blockedUsersDesc: 'Gestisci e sblocca gli utenti che hai bloccato',
        noBlockedUsers: 'Nessun utente bloccato.',
        unblockConfirmTitle: 'Sblocca utente',
        unblockConfirmDesc: 'Sei sicuro di voler sbloccare questo utente?',
        unblockSuccess: 'L\'utente è stato sbloccato.',
        deleteAccountButton: 'Elimina Il Mio Account',
        accountDeleted: 'Il tuo account è stato eliminato.',
        logoutConfirm: 'Sei sicuro di voler uscire?',
        passportHolder: 'Titolare del Passaporto',
        theme: 'Tema',
        lightDark: 'Chiaro / Scuro',
        exitLabel: 'ESCI',
        exportDataTitle: 'Richiedi i miei dati',
        exportDataDesc: 'Per ottenere una copia dei tuoi dati personali, scrivi a privacy@odysseyjournal.app. Esporteremo il tuo profilo, i post, i commenti, le raccolte e le connessioni e te li invieremo.',
        exportDataSubdesc: 'Scrivici per ricevere una copia dei tuoi dati',
        sendEmail: 'Invia email',
        legal: 'LEGALE E COMMUNITY',
        communityGuidelines: 'Linee guida della community',
        communityGuidelinesDesc: 'Regole per mantenere sicura la nostra community',
        adminTitle: 'ADMIN',
        moderationPanel: 'Pannello di Moderazione',
        moderationPanelDesc: 'Rivedi le segnalazioni, gestisci gli utenti',
        likeNotifications: 'Mi piace',
        likeNotificationsDesc: 'Ricevi notifiche quando qualcuno mette mi piace a un tuo post',
        commentNotifications: 'Commenti',
        commentNotificationsDesc: 'Ricevi notifiche quando qualcuno commenta un tuo post',
        followNotifications: 'Nuovi seguaci',
        followNotificationsDesc: 'Ricevi notifiche quando qualcuno inizia a seguirti',
        changePassword: 'Cambia password',
        changePasswordDesc: 'Aggiorna la password del tuo account',
        preferencesUpdateError: 'Si è verificato un errore durante l\'aggiornamento delle preferenze.',
        notificationPreferences: 'Preferenze di Notifica',
        passwordChangeSuccess: 'La password è stata aggiornata con successo.',
        unblockError: 'Impossibile sbloccare l\'utente.',
        build: 'Build',
        restartTitle: 'Riavvio necessario',
        restartMessage: 'Chiudi Odyssey Journal e riaprilo per completare il cambio di direzione del layout.',
    },
    errors: {
        generic: 'Qualcosa è andato storto',
        network: 'Errore di connessione',
        tryAgain: 'Riprova',
        sessionExpired: 'Sessione scaduta',
        notFound: 'Non trovato',
        unauthorized: 'Accesso non autorizzato',
        serverError: 'Errore del server',
        boundaryTitle: 'La nostra bussola si è persa!',
        boundarySubtitle: 'Si è verificato un errore imprevisto. Stiamo lavorando per ripristinare il servizio.',
        boundaryErrorDetails: 'Dettagli dell\'errore',
        boundaryRetry: 'Riavvia l\'applicazione',
        boundaryShowDetails: 'Mostra dettagli dell\'errore',
        boundaryHideDetails: 'Nascondi dettagli dell\'errore',
        noConnection: 'Nessuna connessione Internet',
        permissionPhotos: 'Spiacenti, serve il permesso di accedere alla galleria per caricare le immagini.',
        permissionCamera: 'Spiacenti, serve il permesso della fotocamera per scattare foto.',
        permissionLocation: 'Spiacenti, serve il permesso di posizione per aggiungerla ai tuoi post.',
        imagePickFailed: 'Impossibile selezionare l\'immagine. Riprova.',
        imagesPickFailed: 'Impossibile selezionare le immagini. Riprova.',
        photoCaptureFailed: 'Impossibile scattare la foto. Riprova.',
        locationFailed: 'Impossibile ottenere la posizione attuale. Riprova.',
        signInTitle: 'Errore di accesso',
        signInProviderFailed: 'Si è verificato un problema durante l\'accesso con {{provider}}.',
        shareFailed: 'Impossibile condividere il post',
        emailNotConfirmed: 'Conferma il tuo indirizzo email prima di accedere',
        emailExists: 'Esiste già un account con questa email',
        weakPassword: 'Scegli una password più sicura',
        samePassword: 'La nuova password deve essere diversa da quella attuale',
        rateLimit: 'Troppi tentativi. Attendi un momento e riprova',
        linkExpired: 'Questo link è scaduto. Richiedine uno nuovo',
        userNotFound: 'Nessun account trovato per questo indirizzo email',
        signupDisabled: 'Le nuove registrazioni sono momentaneamente chiuse',
        usernameTaken: 'Questo nome utente è già in uso',
    },
    time: {
        now: 'Adesso',
        minuteAgo: '{{count}} minuto fa',
        minutesAgo: '{{count}} minuti fa',
        hourAgo: '{{count}} ora fa',
        hoursAgo: '{{count}} ore fa',
        dayAgo: '{{count}} giorno fa',
        daysAgo: '{{count}} giorni fa',
        weekAgo: '{{count}} settimana fa',
        weeksAgo: '{{count}} settimane fa',
        monthAgo: '{{count}} mese fa',
        monthsAgo: '{{count}} mesi fa',
        yearAgo: '{{count}} anno fa',
        yearsAgo: '{{count}} anni fa',
        yesterday: 'Ieri',
        lastWeek: 'Settimana scorsa',
    },
    empty: {
        noData: 'Nessun dato',
        noPosts: 'Nessun post',
        noUsers: 'Nessun utente',
        noPlaces: 'Nessun luogo',
    },
    map: {
        title: 'Mappa',
        loading: 'Caricamento mappa...',
        notSupportedWeb: 'Mappa non disponibile sul Web',
        notSupportedExpo: 'Mappa non disponibile in Expo Go',
        useMobileApp: 'Usa l\'app mobile per visualizzare la mappa.',
        useDevBuild: 'Build di sviluppo richiesta per visualizzare la mappa.',
        fallbackList: 'Puoi visualizzare l\'elenco delle posizioni qui sotto.',
        stats: '{{locationCount}} luoghi • {{postCount}} post',
        unknownLocation: 'Posizione sconosciuta',
        postCount: '{{count}} post',
        noPosts: 'Nessun post con posizione',
        addLocationTip: 'Aggiungi posizione ai tuoi post',
        openInMap: 'Apri nella mappa',
        travelPoint: 'Punto di viaggio',
        journeyRoute: 'Itinerario di Viaggio ({{count}} Tappe)',
        noJourneyYet: 'Nessun viaggio ancora',
        startExploring: 'Inizia ad esplorare il mondo!',
        notSupported: 'Mappa non supportata',
        locationVisited: 'Luogo visitato',
        locationsVisited: 'Luoghi visitati',
        appleMaps: 'Apple Mappe',
        openInMaps: 'Mappe',
    },

    destinationPosts: {
        empty: 'Nessun post in questa località',
        emptyDesc: 'Non ci sono ancora post condivisi in questa destinazione',
        destination: 'Destinazione',
        untitledPost: 'Post senza titolo',
        user: 'Utente',
        error: 'Si è verificato un errore',
    },
    followers: {
        title: 'Seguaci',
        noFollowers: 'Ancora nessun seguace',
        noFollowersDesc: 'Quando le persone seguiranno questo utente, appariranno qui',
        loadError: 'Impossibile caricare i seguaci',
        followError: 'Impossibile aggiornare lo stato di follow',
    },
    following: {
        title: 'Seguiti',
        notFollowing: 'Non segue nessuno',
        notFollowingDesc: 'Quando questo utente seguirà delle persone, appariranno qui',
        loadError: 'Impossibile caricare la lista dei seguiti',
        followError: 'Impossibile aggiornare lo stato di follow',
    },
    popularPosts: {
        title: 'Post popolari',
        empty: 'Nessun post popolare',
        emptyDesc: 'Non sono ancora stati trovati post popolari',
        untitledPost: 'Post senza titolo',
        user: 'Utente',
        error: 'Si è verificato un errore',
    },
    createPost: {
        newEntry: 'Nuova voce del diario',
        cancel: 'Annulla',
        share: 'Condividi',
        images: 'IMMAGINI',
        addPhoto: 'Aggiungi foto',
        addNote: 'Aggiungi nota...',
        confirmLocation: 'Conferma posizione',
        changeLocation: 'Modifica',
        titlePlaceholder: 'Titolo',
        storyPlaceholder: 'Inizia a raccontare la tua storia...',
        errorTitle: 'Errore',
        enterTitle: 'Inserisci un titolo',
        enterStory: 'Scrivi la tua storia',
        success: 'Successo',
        entryCreated: 'La tua voce del diario è stata creata!',
        ok: 'OK',
        createError: 'Impossibile creare il post. Riprova.',
        addPhotoTitle: 'Aggiungi foto',
        chooseOption: 'Scegli un\'opzione',
        takePhoto: 'Scatta foto',
        pickFromGallery: 'Scegli dalla galleria',
        nature: 'Natura',
        city: 'Città',
        food: 'Cibo',
        history: 'Storia',
        art: 'Arte',
        emailNotVerifiedTitle: 'Verifica email richiesta',
        emailNotVerifiedDesc: 'Verifica il tuo indirizzo email per condividere nuove voci di diario con la community.',
    },

    weather: {
        clear: 'Sereno',
        mainlyClear: 'Prevalentemente Sereno',
        partlyCloudy: 'Parzialmente Nuvoloso',
        overcast: 'Nuvoloso',
        foggy: 'Nebbioso',
        depositingRimeFog: 'Nebbia con Brina',
        lightDrizzle: 'Pioggerella Leggera',
        moderateDrizzle: 'Pioggerella Moderata',
        denseDrizzle: 'Pioggerella Intensa',
        freezingDrizzle: 'Pioggerella Gelata',
        denseFreezingDrizzle: 'Pioggerella Gelata Intensa',
        slightRain: 'Pioggia Leggera',
        moderateRain: 'Pioggia Moderata',
        heavyRain: 'Pioggia Forte',
        freezingRain: 'Pioggia Gelata',
        heavyFreezingRain: 'Pioggia Gelata Forte',
        slightSnow: 'Neve Leggera',
        moderateSnow: 'Neve Moderata',
        heavySnow: 'Neve Forte',
        snowGrains: 'Granelli di Neve',
        slightRainShowers: 'Rovesci Leggeri',
        moderateRainShowers: 'Rovesci Moderati',
        violentRainShowers: 'Rovesci Violenti',
        slightSnowShowers: 'Neve Leggera',
        heavySnowShowers: 'Neve Forte',
        thunderstorm: 'Temporale',
        thunderstormSlightHail: 'Temporale con Grandine Leggera',
        thunderstormHeavyHail: 'Temporale con Grandine Forte',
        unknown: 'Sconosciuto',
    },
    guidelines: {
        intro: 'Benvenuto su Odyssey Journal! Vogliamo mantenere questo spazio sicuro, stimolante e incentrato su esperienze di viaggio autentiche. Ti preghiamo di seguire queste linee guida.',
        respectTitle: 'Rispetta la community',
        respectDesc: 'Odyssey Journal è un luogo in cui i viaggiatori possono condividere le loro vere esperienze. Tratta gli altri con rispetto. Incitamento all\'odio, molestie, bullismo e discriminazioni di qualsiasi tipo sono severamente vietati.',
        authenticTitle: 'Condividi viaggi autentici',
        authenticDesc: 'Concentrati sui viaggi, i luoghi, la cultura e il tuo viaggio personale. Non pubblicare spam, pubblicità, propaganda politica o contenuti non correlati.',
        safeTitle: 'Sicuro e appropriato',
        safeDesc: 'Non pubblicare nudità, contenuti sessualmente espliciti o violenza grafica. Assicurati che foto e testi siano appropriati per un pubblico generale.',
        privacyTitle: 'Rispetta la privacy',
        privacyDesc: 'Non condividere le informazioni personali di altri senza il loro esplicito consenso. Fai attenzione a chi e cosa includi nelle tue foto.',
        enforcementTitle: 'Applicazione',
        enforcementDesc: 'Esaminiamo i contenuti e gli account segnalati. La violazione di queste linee guida può comportare la rimozione dei contenuti, limitazioni di accesso o l\'eliminazione permanente dell\'account senza preavviso.',
        thankYou: 'Grazie per aiutarci a rendere Odyssey Journal un posto fantastico per i viaggiatori!'
    },
    // Content Moderation
    moderation: {
        flaggedNoReasons: 'Il tuo contenuto è stato segnalato per la revisione. Assicurati che rispetti le nostre linee guida della community.',
        flaggedWithReasons: 'Il tuo contenuto è stato segnalato per: {{reasons}}. Assicurati che rispetti le nostre linee guida della community.',
        categories: {
            harassment: 'molestie',
            'harassment/threatening': 'molestie minacciose',
            hate: 'discorso d\'odio',
            'hate/threatening': 'odio minaccioso',
            'self-harm': 'autolesionismo',
            'self-harm/intent': 'intento di autolesionismo',
            'self-harm/instructions': 'istruzioni di autolesionismo',
            sexual: 'contenuto sessuale',
            'sexual/minors': 'contenuto sessuale di minori',
            violence: 'violenza',
            'violence/graphic': 'violenza grafica'
        }
    },
    homeLocation: {
        title: 'Imposta la tua città di partenza',
        description: 'Imposta la tua città di partenza per consentirci di calcolare con precisione la distanza totale percorsa e le statistiche del tuo passaporto.',
        useCurrentLocation: 'Usa la mia posizione attuale',
        locating: 'Rilevamento posizione...',
        later: 'Più tardi',
        success: 'Città di partenza impostata con successo.',
        permissionDenied: 'Autorizzazione alla posizione negata. Puoi impostarla in seguito dal tuo profilo.',
        error: 'Impossibile rilevare la posizione.',
    },
    badgeList: {
        firstAdventure: {
            name: "Prima Avventura",
            description: "Condividi il tuo primo post"
        },
        worldTraveler: {
            name: "Viaggiatore del Mondo",
            description: "Visita 5 paesi diversi"
        },
        photographer: {
            name: "Maestro Fotografo",
            description: "Condividi 10 post con foto"
        },
        gourmet: {
            name: "Buongustaio",
            description: "Condividi 3 post nella categoria cibo"
        },
        marathonTraveler: {
            name: "Viaggiatore Maratona",
            description: "Percorri 10.000 km"
        },
        explorer: {
            name: "Esploratore",
            description: "Viaggia per 30 giorni"
        }
    },
    // Date Picker
    datePicker: {
        title: 'DATA / ORA DI VIAGGIO',
        tapToChange: 'Tocca per modificare la data',
        change: 'Modifica',
        today: 'Oggi',
        yesterday: 'Ieri',
        tomorrow: 'Domani',
        threeDaysAgo: '3 giorni fa',
        calendar: 'Calendario',
        selectDate: 'Seleziona data del viaggio',
        selected: 'Selezionato:',
        apply: 'Applica data',
        cancel: 'Annulla',
        daysAgo: '{{count}} giorni fa',
        daysLater: 'tra {{count}} giorni',
    },

    // Admin panel
    admin: {
        panelTitle: 'PANNELLO DI AMMINISTRAZIONE',
        accessDenied: 'Accesso negato',
        noPrivileges: 'Non hai privilegi di amministratore.',
        goBack: 'Torna indietro',
        all: 'Tutti',
        pending: 'In attesa',
        resolved: 'Risolti',
        dismissed: 'Ignorati',
        total: 'Totale',
        banned: 'Bloccati',
        reporterNote: 'Nota di chi ha segnalato:',
        reportedBy: 'Segnalato da: {{name}}',
        anonymous: 'Anonimo',
        unknownUser: 'Sconosciuto',
        dismiss: 'Ignora',
        deletePost: 'Elimina post',
        deletePostConfirm: 'Questo post verrà eliminato definitivamente. Sei sicuro?',
        deletePostDone: 'Post eliminato e segnalazione risolta.',
        deletePostFailed: 'Impossibile eliminare il post.',
        banUser: 'Blocca utente',
        ban: 'Blocca',
        banUserConfirm: 'Bloccare {{username}}? Non potrà più creare post o commenti.',
        banUserDone: '{{username}} è stato bloccato.',
        banUserFailed: 'Impossibile bloccare l\'utente.',
        dismissFailed: 'Impossibile ignorare la segnalazione.',
        noReports: 'Nessuna segnalazione trovata',
        allCaughtUp: 'Tutto in ordine! Nessuna segnalazione in attesa.',
        noMatchingReports: 'Nessuna segnalazione corrisponde a questo filtro.',
    },

    // Modal screen
    modal: {
        title: 'Modale',
        heading: 'Questa è una finestra modale',
        goHome: 'Vai alla schermata iniziale',
    },
};
