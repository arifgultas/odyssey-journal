# Odyssey Journal - Web Sitesi İçerikleri

Aşağıdaki metinleri kopyalayıp web sitenizdeki ilgili sayfalara yapıştırabilirsiniz.

---

## 1. Privacy Policy (Gizlilik Politikası)
**Link:** `arifgultas.github.io/odyssey-journal/privacy-policy.html`

```markdown
# Privacy Policy for Odyssey Journal

**Last Updated: January 4, 2026**

## Introduction

Welcome to Odyssey Journal ("we," "our," or "us"). We respect your privacy and are committed to protecting your personal data. This privacy policy explains how we collect, use, and safeguard your information when you use our mobile application.

## Information We Collect

### Information You Provide

- **Account Information**: Email address, username, full name, profile picture
- **Content**: Posts, photos, comments, and location data you choose to share
- **Profile Information**: Bio, avatar, and other profile details

### Automatically Collected Information

- **Usage Data**: How you interact with the app, features you use
- **Device Information**: Device type, operating system, unique device identifiers
- **Location Data**: GPS coordinates when you tag posts (only with your permission)

## How We Use Your Information

We use your information to:

- Provide and maintain our service
- Create and manage your account
- Display your posts and profile to other users
- Send notifications about likes, comments, and follows
- Improve and optimize our app
- Ensure security and prevent fraud
- Comply with legal obligations

## Data Sharing

We do NOT sell your personal information. We may share data with:

- **Other Users**: Your public profile and posts are visible to other users
- **Service Providers**: Supabase (database), Google Maps (location services)
- **Legal Requirements**: When required by law or to protect our rights

## Data Storage and Security

- Your data is stored securely using Supabase
- We use encryption for data transmission
- We implement industry-standard security measures
- We retain your data as long as your account is active

## Your Rights

You have the right to:

- Access your personal data
- Correct inaccurate data
- Delete your account and data
- Export your data
- Opt-out of certain data collection
- Withdraw consent at any time

## Children's Privacy

Our service is not intended for users under 13 years of age. We do not knowingly collect data from children under 13.

## Third-Party Services

We use the following third-party services:

- **Supabase**: Database and authentication
- **Google Maps**: Location services
- **Expo**: App development platform

Each service has its own privacy policy.

## Cookies and Tracking

We use local storage and caching to improve app performance. We do not use cookies for tracking or advertising.

## Changes to This Policy

We may update this privacy policy from time to time. We will notify you of significant changes through the app or via email.

## Contact Us

If you have questions about this privacy policy, please contact us at:

- **Email**: gultassoftware@gmail.com
- **Website**: https://arifgultas.github.io/odyssey-journal/privacy-policy.html

## Data Protection Rights (GDPR)

If you are in the European Economic Area (EEA), you have additional rights under GDPR:

- Right to access
- Right to rectification
- Right to erasure
- Right to restrict processing
- Right to data portability
- Right to object
- Rights related to automated decision-making

To exercise these rights, contact us at gultassoftware@gmail.com

## California Privacy Rights (CCPA)

If you are a California resident, you have the right to:

- Know what personal information is collected
- Know whether personal information is sold or disclosed
- Say no to the sale of personal information
- Access your personal information
- Request deletion of personal information
- Not be discriminated against for exercising your rights

---
**By using Odyssey Journal, you agree to this Privacy Policy.**
```

---

## 2. Terms of Service (Kullanım Koşulları)
**Link:** `arifgultas.github.io/odyssey-journal/terms-of-service.html`

```markdown
# Terms of Service for Odyssey Journal

**Last Updated: January 4, 2026**

## 1. Acceptance of Terms

By accessing and using Odyssey Journal ("the App"), you accept and agree to be bound by these Terms of Service ("Terms"). If you do not agree to these Terms, please do not use the App.

## 2. Description of Service

Odyssey Journal is a mobile application that allows users to:

- Create and share travel posts with photos and locations
- Discover and interact with posts from other travelers
- Follow other users and build a travel community
- Bookmark and organize favorite posts
- Receive notifications about social interactions

## 3. User Accounts

### 3.1 Account Creation

- You must be at least 13 years old to use the App
- You must provide accurate and complete information
- You are responsible for maintaining account security
- You must not share your account credentials

### 3.2 Account Termination

We reserve the right to suspend or terminate accounts that:

- Violate these Terms
- Engage in abusive or harmful behavior
- Post inappropriate or illegal content
- Impersonate others or create fake accounts

## 4. User Content

### 4.1 Your Content

- You retain ownership of content you post
- You grant us a license to display, distribute, and promote your content
- You are responsible for the content you post
- You must have rights to any photos or content you share

### 4.2 Prohibited Content

You may not post content that:

- Is illegal, harmful, or offensive
- Infringes on intellectual property rights
- Contains personal information of others without consent
- Promotes violence, hate speech, or discrimination
- Is sexually explicit or pornographic
- Contains spam or malicious code

### 4.3 Content Moderation

We reserve the right to:

- Remove content that violates these Terms
- Moderate user-generated content
- Take action against users who violate policies

## 5. User Conduct

You agree to:

- Use the App lawfully and respectfully
- Not harass, bully, or threaten other users
- Not impersonate others
- Not attempt to hack or disrupt the App
- Not scrape or collect data without permission
- Not use the App for commercial purposes without authorization

## 6. Intellectual Property

### 6.1 Our Rights

- The App, design, and features are owned by us
- Our trademarks and logos are protected
- You may not copy, modify, or distribute our intellectual property

### 6.2 User Rights

- You retain rights to your original content
- You grant us a non-exclusive, worldwide license to use your content
- This license ends when you delete your content or account

## 7. Privacy

Your privacy is important to us. Please review our Privacy Policy to understand how we collect, use, and protect your information.

## 8. Disclaimers

### 8.1 Service Availability

- The App is provided "as is" without warranties
- We do not guarantee uninterrupted or error-free service
- We may modify or discontinue features at any time

### 8.2 User Content

- We are not responsible for user-generated content
- We do not endorse user opinions or content
- Users are solely responsible for their content

### 8.3 Third-Party Services

- We use third-party services (Supabase, Google Maps)
- We are not responsible for third-party service failures
- Third-party services have their own terms and policies

## 9. Limitation of Liability

To the maximum extent permitted by law:

- We are not liable for indirect, incidental, or consequential damages
- Our total liability is limited to the amount you paid us (if any)
- We are not liable for user content or actions
- We are not liable for service interruptions or data loss

## 10. Indemnification

You agree to indemnify and hold us harmless from:

- Claims arising from your use of the App
- Your violation of these Terms
- Your violation of others' rights
- Your content and actions

## 11. Changes to Terms

We may update these Terms at any time. We will notify you of significant changes through:

- In-app notifications
- Email notifications
- Updated "Last Updated" date

Continued use of the App after changes constitutes acceptance.

## 12. Termination

### 12.1 By You

You may delete your account at any time through the App settings.

### 12.2 By Us

We may terminate or suspend your account if you:

- Violate these Terms
- Engage in prohibited conduct
- Pose a security or legal risk

## 13. Dispute Resolution

### 13.1 Governing Law

These Terms are governed by the laws of the Republic of Turkey.

### 13.2 Arbitration

Any disputes will be resolved through binding arbitration, except where prohibited by law.

### 13.3 Class Action Waiver

You agree to resolve disputes individually, not as part of a class action.

## 14. General Provisions

### 14.1 Entire Agreement

These Terms constitute the entire agreement between you and us.

### 14.2 Severability

If any provision is found invalid, the remaining provisions remain in effect.

### 14.3 No Waiver

Our failure to enforce any right does not waive that right.

### 14.4 Assignment

You may not assign these Terms. We may assign them without notice.

## 15. Contact Us

If you have questions about these Terms, contact us at:

- **Email**: gultassoftware@gmail.com
- **Website**: https://arifgultas.github.io/odyssey-journal/terms-of-service.html

## 16. Special Provisions

### 16.1 Apple App Store

If you download the App from the Apple App Store, you acknowledge that:

- These Terms are between you and us, not Apple
- Apple has no obligation to provide support
- Apple is not responsible for the App
- You comply with Apple's terms of service

### 16.2 Google Play Store

If you download the App from Google Play, you acknowledge that:

- These Terms are between you and us, not Google
- Google has no obligation to provide support
- Google is not responsible for the App
- You comply with Google's terms of service

---
**By using Odyssey Journal, you agree to these Terms of Service.**
```
